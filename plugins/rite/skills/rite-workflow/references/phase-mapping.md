...
`/rite:issue-start` (廃止済み) は新 4 コマンド `/rite:open` / `/rite:iterate` / `/rite:ready` / `/rite:merge` に分解された。本 mapping は work memory `フェーズ詳細` 欄および `flow-state.json` の `phase` field の表示文字列を定義する。
...
|-------|-------------|------------|
| `init` | Issue 取得・親子判定 | `/rite:open` ステップ 1 |
| `branch` | ブランチ作成完了 | `/rite:open` ステップ 2 |
| `plan` | 実装計画生成完了 | `/rite:open` ステップ 3 |
| `implement` | 実装作業中 / 完了 | `/rite:open` ステップ 4 (内部で `rite:issue-implement` invoke) |
| `lint` | 品質チェック完了 | `/rite:open` ステップ 5 |
| `pr` | PR 作成完了 | `/rite:open` ステップ 6 |
| `review` | レビュー実施中 / 完了 | `/rite:iterate` review side |
| `fix` | レビュー修正中 / 完了 | `/rite:iterate` fix side |
