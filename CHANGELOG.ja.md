...
- **`setup` が `multi_session` + sandbox 環境特有の 2 つの既知環境制約を、失敗発生後ではなく `/rite:setup` 実行時点で事前検出・案内するようになった** — (1) Phase 4.8: `EnterWorktree` 後、セッション worktree の cwd から main checkout 配下への state 書込（`flow-state.sh` / `issue-claim.sh` 等）が拒否される制約、(2) Phase 4.9: sandbox 有効セッションが SSH host alias remote（例: `git@github.com-work:owner/repo.git`）を持つ場合に `git push`/`fetch` が Bad Gateway で失敗する制約。sandbox の有効判定は bash から検出不能なため、両 Phase とも同じ方式（Claude 自身の実行コンテキスト判定）を採る。(#1907, #1925, #1938)
