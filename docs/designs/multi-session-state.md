...
- `error_count` (`flow-state-update.sh` patch mode が phase transition 時に `0` リセット writer として書込。`--preserve-error-count` 指定の self-patch は既存値を保持。**incrementer は PR #675 で撤去された stop-guard.sh のみで、現行 production code に incrementer 経路は存在しない** — incrementer 不在の半-legacy field。S3 で schema 維持するか除外するかは後追い検討事項)
...
| `loop_count` field | reader 経路 (`post-compact.sh` / `session-start.sh` / `state-read.sh`) は存在するが、`flow-state-update.sh` 内に **`loop_count` への writer は実装されていない** (`grep -n loop_count plugins/rite/hooks/flow-state-update.sh` → 0 hits)。`work-memory-update.sh` の `loop_count` 増分は work-memory ドキュメント側のフィールドを更新するもので flow-state の `loop_count` には書き戻されない。Phase 1 設計時 (本ドキュメント上記 Option A 「実行時に追加される optional フィールド」note) と整合する状態のまま、reader-only legacy field として schema slot を維持し将来 writer 実装に備える |
| `error_count` field | 現行 production code に **incrementer 経路は存在しない**（`stop-guard.sh` は PR #675 で撤去済み）。`flow-state-update.sh` patch mode が phase transition 時に `0` リセット writer として書込（`--preserve-error-count` 指定の self-patch は既存値を保持）。incrementer 不在の half-legacy field として schema は維持し、将来再導入される可能性に備える |
