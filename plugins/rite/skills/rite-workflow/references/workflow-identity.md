...
**Summary**: sub-skill (`lint` / `pr:review` / `pr:fix` / `pr:ready` / `pr:cleanup` / `wiki:ingest` 等) が return tag (`[lint:success]`, `[review:mergeable]`, `[fix:pushed]`, `[ready:completed]`, `[ingest:completed]` 等) を出力して制御を返した時、LLM は **同 turn 内で** orchestrator の `🚨 Mandatory After ...` セクションを実行して次 phase へ継続すること。return tag は turn 境界ではなく **継続トリガ** である。

> **Historical (the flat-workflow refactor)**: 以前は `/rite:issue:create` 配下に `create-interview` / `create-register` / `create-decompose` の 3 sub-skill が存在し、return tag (`[interview:completed]`, `[create:completed:{N}]`) も契約に含まれていた。現在 `create.md` 内部に閉じている。`[create:completed:{N}]` sentinel 自体は terminal output として残るが、内部 chain 跨ぎの continuation トリガとしての役割は撤去済。

**Failure Patterns (Anti-pattern)**:
- sub-skill が `[lint:success]` / `[review:mergeable]` / `[ingest:completed]` を出力して return した直後に LLM が turn を閉じ、ユーザーが `continue` 入力を余儀なくされる
...
3. Stop の防御は (a) `commands/...` の prompt contract、(b) `flow-state.sh` の phase 記録 (`PHASE_ENUM_V3` + `_phase_is_valid`)、(c) `pre-tool-bash-guard.sh` の事前ガードに分散されている。`end_turn` を機械的に block する Stop hook は存在せず、protocol violation の検出は caller orchestrator (sprint flow 等) の sequential 実行末尾 / `/rite:resume` re-entry / `/rite:pr:cleanup` 完了 報告で retrospective に grep 検出する。
...
**Summary**: `/rite:issue:create` / `/rite:pr:open` / `/rite:pr:iterate` / `/rite:pr:ready` / `/rite:pr:merge` など一連のワークフロー完了時、ユーザーに見える最終行は `[create:completed:{N}]` のような sentinel marker ではなく、`✅ Issue #{N} を作成しました: {url}` のような「完了したと即座に理解できる」人間可読メッセージであること。sentinel marker は hook/grep 契約のため出力自体は保持するが、HTML コメント化 (`<!-- [create:completed:{N}] -->`) 等で user-visible な末端に孤立させない。
...
**Correct Pattern**:
- `commands/issue/create.md` ステップ 4.4 (Single Issue 完了レポート) / ステップ 5.6 (Decompose 完了レポート) の出力テンプレートを「✅ 完了メッセージ → 次のアクション → `<!-- [create:completed:{N}] -->` (HTML コメント最終行) → ステップ 6 flow-state 完結」の順序で統一する。
...
実 caller 配備の現状: 旧 `cleanup.md Phase 4.W.2` の blockquote style は Issue #1144 / PR #1149 の defense 層物理排除で削除済。現在の caller は `inline` style (`Identity: [workflow-identity.md](...)。`) のみで配備されている (`grep -rn 'workflow-identity' plugins/rite/commands/` で確認可)。新規 caller は本 Example の blockquote style か、既存の inline style のいずれかを選択する。

**既存 variance の扱い**: 既存 caller の 2 style (blockquote / inline) はそのまま維持してよい (retrospective な書き換えは scope creep)。新規追加時のみ本 template に従うこと。
