...
> **Canonical SoT**: 登録されている hook event の権威リストは [`plugins/rite/hooks/hooks.json`](../plugins/rite/hooks/hooks.json) にある。本テーブルはその登録内容のミラーであり、両者が乖離した場合は `hooks.json` が優先される。読者の便宜のため列挙しているが、登録内容が変わったら必ず `hooks.json` のキー（`jq '.hooks | keys[]' plugins/rite/hooks/hooks.json`）から再生成すること。
...
1. per-session compact-state（`.rite/sessions/{session_id}.compact-state`。session id が解決できない場合のみレガシー共有 `.rite-compact-state` にフォールバック）を読み取る（ファイルが存在しない場合は許可）
...
1. 解決された state root 配下の per-session compact-state（`.rite/sessions/{session_id}.compact-state`。per-session flow-state パスから導出）と per-session flow state ファイルを読む（`state-path-resolve.sh` に解決を委譲。[マルチセッション状態管理](#マルチセッション状態管理)参照）
2. flow state が存在しない場合、per-session compact-state をクリーンアップして exit 0 (孤立した compact marker の自己修復)
...
ワークフローが終了しているにもかかわらず per-session compact-state が残存している場合（クラッシュなど）、自動的にクリーンアップして silent に exit し、新規セッションの妨害を防ぐ。`session-start.sh` は加えてレガシー共有 `.rite-compact-state` を per-session 化以前の残骸の移行経路として reap する。
...
2. per-session flow state ファイル（`.rite/sessions/{session_id}.flow-state`）からアクティブなワークフローと Issue 番号を取得
...
| `WM_PHASE` | はい | 現在のフェーズ（`lint`, `implement`, `pr` 等。`flow-state.sh` の `PHASE_ENUM_V3` 参照） |
...
| `get_state_session_id <file>` | per-session flow state ファイルから `session_id` を読む |
...
- compact 状態は per-session の `.rite/sessions/{session_id}.compact-state` で管理（レガシー共有 `.rite-compact-state` は移行用に維持）
- `blocked` 状態では `/rite:resume` 以外のすべてのコマンドをブロック
- `/clear` → `/rite:resume` で正常状態に復帰

### マルチセッション状態管理

> **設計の根拠**: 設計選定の全体（6 軸トレードオフ比較、Option A vs B の Decision Log、Phase 2 実装の振り返り）は [`docs/designs/multi-session-state.md`](designs/multi-session-state.md) を参照。本節は権威ある**ランタイム仕様**であり、設計ドキュメントは権威ある**根拠**の記録である。
...
`session_id` は `.rite-session-id` に保存されているのと同じ UUID で、JSON stdin ペイロードを介してすべての hook に伝播される。

**スキーマ（`schema_version: 3`）:**

| カテゴリ | フィールド | ソース / Writer | 備考 |
|----------|-----------|-----------------|------|
| 必須 (10) | `active` | `flow-state.sh set` | ワークフロー実行中は `true` |
| 必須 | `issue_number` | `flow-state.sh set` | 作業対象の Issue |
| 必須 | `branch` | `flow-state.sh set` | フィーチャーブランチ名 |
| 必須 | `phase` | `flow-state.sh set` | 現在の orchestrator phase（flat enum: `init` / `branch` / `plan` / `implement` / `lint` / `pr` / `review` / `fix` / `ready` / `ready_error` / `cleanup` / `ingest` / `completed`） |
| 必須 | `pr_number` | `flow-state.sh set` | PR 作成までは `0` |
| 必須 | `parent_issue_number` | `flow-state.sh set` | Issue が単独の場合は `0` |
| 必須 | `next_action` | `flow-state.sh set` | post-compact 復旧で表面化される自由記述の継続ヒント |
| 必須 | `updated_at` | `flow-state.sh set`（書き込みごと） | `Z` サフィックス付き ISO 8601 UTC。`date -u +"%Y-%m-%dT%H:%M:%SZ"`（クロスプラットフォームで決定的）で生成。注: 他所の人間向けログは JST の場合があるが、永続化される state フィールドは UTC |
...
> **`previous_phase` フィールド**: v2→v3 移行で撤去済み。v2 スキーマは離脱する `phase` 値から自動補完していたが、v3 は resume routing を step 名マッピング（`commands/resume.md`）で判別する。`cmd_set` はもうこれを書かず（`grep -n previous_phase plugins/rite/hooks/flow-state.sh` で検証）、`_migrate_file` は `del(.previous_phase)` で移行ファイルから除去する。

**legacy 単一ファイル形式からの移行:**

legacy state ファイル（`schema_version` なしの flat JSON、または `schema_version != 3` の任意のファイル）は、セッション開始時に [`flow-state.sh migrate`](../plugins/rite/hooks/flow-state.sh) — `cmd_migrate` / `_migrate_file` パス — によって自動的に v3 へ移行される（[`session-start.sh`](../plugins/rite/hooks/session-start.sh) から invoke される）。`_migrate_file` は各ファイルを `mktemp + flock + atomic mv`（`_atomic_write`）で**インプレース**書き換えする: legacy の `previous_phase` フィールドを除去し、`branch_name` → `branch` に正規化し、legacy の `phase` 値を v3 enum に縮約し、`schema_version` を `3` にバンプし、`updated_at` を更新する一方で `last_synced_phase` は保持する。別途の `.rite-flow-state.legacy.{timestamp}` バックアップは存在しない — 書き換えはインプレースである。移行が行われた場合は常に明示的な `migrated:` 行を stderr に出力する（`--verbose` に依存しない無条件出力。session-start の自動パスで表面化させるため — silent skip は禁止、AC-8）。no-op の既に v3 であるケースは `--verbose` がない限り静かなまま。`--dry-run` プレビュー（`would migrate:`）も `migrated:` アナウンスとの対称性のため stderr に出力され、session-start の stdout-only サイレンスポリシー下でも dry-run 出力が実際の移行と並んで表面化する。phase 縮約マトリクスと手動 `--dry-run` フォールバックは [`docs/migration-guides/v2-to-v3.md`](migration-guides/v2-to-v3.md) に、マルチセッションの atomicity / glob 衝突の根拠は [`docs/designs/multi-session-state.md`](designs/multi-session-state.md#migration-戦略) に記載。

**ロールバック戦略:**

`rite-config.yml` は `flow_state.schema_version: 1` を受け付けて legacy コードパスを強制できる（アダプタパターン）。この二重ロジックは soak 期間後に削除される予定（目標: v0.5.0）。

**Sub-Issues API による親子構造:**
...
| フロー状態 | ワークフロー制御 | `.rite/sessions/{session_id}.flow-state`（per-session。[マルチセッション状態管理](#マルチセッション状態管理)参照） |
| Compact 状態 | compact 後の状態管理 | `.rite/sessions/{session_id}.compact-state`（per-session。レガシー共有 `.rite-compact-state` は移行用に維持） |

**ローカル作業メモリの特徴:**

- `mkdir` ベースの排他ロックで同時アクセスを制御
- PostToolUse フックによる自動復旧
- compact 後も per-session flow state ファイルから状態を復元可能
...
| `post-compact.sh` | recovery context の出力、per-session compact-state の自己修復 |
...
- 作業メモリ（Issue コメント + ローカルの `.rite-work-memory/issue-{n}.md` ファイル）と git/PR 成果物が、ワークフロー状態をセッション間で永続化する。per-session flow state ファイルは session-scoped であり（[マルチセッション状態管理](#マルチセッション状態管理)参照）、`/clear` 後のセッションは新しい空のファイルを読む。`/rite:resume` は作業メモリ + git/PR のクロスチェックから再開点を再構成し、flow state は存在する場合に限り同一セッションのシグナルとして使用する
...
| ドラフト PR | GitHub | はい |
| 作業メモリ | Issue コメント | はい |
| フロー状態 | `.rite/sessions/{session_id}.flow-state`（[マルチセッション状態管理](#マルチセッション状態管理)参照） | 部分的 — ファイルはディスク上に残るが、`/clear` 後のセッションは新しい空のファイルを読む（session-scoped）。`/rite:resume` は作業メモリ + git/PR にフォールバックする |
