...
#
# Known limitation 2 (M-1 PR #926 verified-review): 各 grep の `2>/dev/null` 経路で
...
# IMP-3 対応 (PR #926 verified-review): `--if-exists` flag の silent revert を catch する pin。
# parent-routing pattern で導入された file 不在時 silent skip guard (`flow-state-update.sh:439,459`)
# を defeat する revert (例: `--if-exists` を一括削除 / `--preserve-error-count` に書き換え) を
# 検出する。実測: Pre-flight patch (3 site) + Return Output re-patch (1 site) = 6 occurrences。
# 最低 3 を要求して将来の正当な refactor (1 site のみ撤廃等) でも catch する。
...
# C-2 対応 (PR #926 verified-review): hint message を実測値に更新。
...
# M-7 対応 (PR #926 verified-review): silent failure pattern の variation を網羅的に検出。
