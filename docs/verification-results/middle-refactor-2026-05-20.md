...
| 4 | #1004 findings 再現 | 6 件中 ≥4 件が nit-noted、Issue 化 0 | PR #1004 (MERGED、reviews count 0) は live review データなし。Issue body 期待値の根拠データに access 不可 — S5/S6/S13 のロジック追跡から「reviewer が同 PR で 6 件の指摘を CRITICAL/HIGH なしで出力した場合、auto_demote_low + scope=nit-noted 降格で ≥4 件 nit-noted 期待動作」は trace で再現可能 | ⚠️ N/A → 静的 trace で代替 (PR #1004 reviews 不在) |
...
| AC-5 | 検証完了後、`rite-config.yml` の `review.scope_assignment.enabled` を `true` に bump | ⏳ 本 doc コミット後の最終コミットで実行予定 |
...
項目 4 の本来の意図 (verified-review が出した 6 件の findings を再 review し、scope_assignment 有効化下で ≥4 件が nit-noted に降格することを観測) は dry-run で再現不可。代替として S5/S6/S13 のロジック追跡から「CRITICAL/HIGH なし × LOW or MEDIUM の 6 件は auto_demote_low + scope=nit-noted で ≥4 件 nit-noted 降格」は構造的に成立する。
...
- **schema-doc L25**: 受理 3 値 `1.0.0` / `1.0` legacy alias / `1.1.0`、3 sites case 文同期 (L35)
...
| AC-5 enabled bump | ⏳ 本 doc コミット直後の最終コミットで実行 |

検証完了、Epic #1015 (M1+M2+M5) の構造改修は仕様レベルで pass。次の最終コミットで `rite-config.yml` の `review.scope_assignment.enabled: false → true` を実行する。

## 4. Live 検証データ (dogfooding post-hoc)

本 Sub-Issue 自身を `/rite:issue:start 1022` で E2E 実行した review-fix サイクルが、項目 1/2/3 の運用検証データとして機能する。本セクションは PR 作成後・レビュー実行後に追記する想定 (post-hoc evaluation)。

- PR #: _TBD (本 Sub-Issue から作成される PR)_
- review cycle 数: _TBD_
- nit reply 件数: _TBD_
- accept (認知のみ) 件数: _TBD_
- 別 Issue 化件数: _TBD_

(本 Sub-Issue の最終マージ前に追記すること)
