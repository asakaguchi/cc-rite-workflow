...
- **`setup` Phase 4.8 が、sandbox write 許可リストの設定をユーザーへの案内のみから自動設定に変更** — `multi_session` が有効かつ filesystem write 制限のある sandbox を検出した場合、`/rite:setup` が main checkout root の絶対パスを `.claude/settings.local.json`（gitignore 対象の開発者ローカル設定。コミットされる `.claude/settings.json` は変更しない）の `sandbox.filesystem.allowWrite` へ idempotent に自動追記する。0.9.0 の「既知の制約」に記載された手動設定の負担を解消する。書き込みに失敗した場合（読み取り専用ファイルシステム等）は従来の手動案内メッセージにフォールバックする。(#1942)
