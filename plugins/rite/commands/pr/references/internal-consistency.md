...
>
> **Canonical category names** (literal-substring matched by `commands/pr/review.md` ステップ 5.1.3 META check — 表記揺れは `doc_heavy_post_condition: warning` false positive の原因):
>
...
| 表記 | 用法 | 出現箇所 |
|------|------|----------|
| `Doc-Heavy PR Mode` | **固有名詞**: `commands/pr/review.md` ステップ 1.2.7 で判定される機能名 | 見出し / 文中の機能名参照 (英) |
...
> **Single source of truth**: skip/activation 判定は [`commands/pr/review.md`](../review.md) ステップ 1.2.7 の `{doc_heavy_pr}` 計算結果に完全委譲（二重定義 drift 防止）。`review.doc_heavy.enabled: false` / 空 PR / rite plugin 自身のドキュメントのみ変更 (`commands/**/*.md`, `skills/**/*.md`, `agents/**/*.md`, `plugins/rite/i18n/**` — 分子から除外、分母には含める方式) / doc 比率閾値未満のいずれでも `doc_heavy_pr = false` で本プロトコル非発動。
