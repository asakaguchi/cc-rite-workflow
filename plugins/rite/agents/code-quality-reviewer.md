...
6. **Cross-domain catch-all for uncategorized issues**: Specialist reviewers (security, application, error-handling, etc.) cover their domains rigorously, but some cross-cutting issues fall between the seams. When you encounter a clear code-quality problem that does NOT obviously belong to another specialist's domain, report it here rather than letting it slip through the cracks. Catch-all targets are **intentionally narrow** (to avoid overreach into specialist domains and keep the catch-all from becoming a dumping ground) and limited to:

   - **Flow control bugs not caught by application-reviewer's type-design lens**: Unreachable code after unconditional `return` / `throw` / `exit`, missing guards that leave a later branch unreachable, dead `else` arms, switch cases with no `break` that silently fall through when they should not
