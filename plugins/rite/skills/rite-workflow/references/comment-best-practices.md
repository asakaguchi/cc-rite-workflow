...
| 旧版表現 | `旧実装は ...` / `旧コードでは ...` / `Previously ...` / `In the old code ...` 等、過去の実装と比較する記述 (本原則 Failure Patterns の用例 `# 旧実装は ～。PR #N で修正` と対応) |
...
> **Maintenance Invariant (parity 保持)**: 原則 2 の row 2 正規表現 (cycle / finding ID 参照、Issue / PR 参照、commit 参照、日本語版、旧版表現) は **§B 原則 2 内「禁止句リスト (SoT)」表と集合等価** を維持する。SoT に新カテゴリを追加する際は本表 row 2 の regex も同期更新し、両者の差集合が常に空であることを保証する。両者が drift すると、生成側 (LLM 駆動の fix.md gate) と reviewer 側 (regex 駆動の tech-writer.md heuristic) の検出範囲が乖離する。
