...
> **その後の経緯 (#1136)**: PR 2b 適用後、`start.md` 自体も #1136 で 4 つの責務単位コマンド (`/rite:pr:open` / `/rite:pr:iterate` / `/rite:pr:ready` / `/rite:pr:merge`) に分解され削除されました。本ガイドの「ステップ 8.5」記述は撤去当時の文脈を示す historical reference であり、新コマンド体系で同等の文脈を辿りたい場合は `/rite:pr:open` Step 5 + `commands/resume.md` の sentinel 検証経路を参照してください。
