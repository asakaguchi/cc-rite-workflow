...
2. 書く WHY は次のいずれかに該当するもの:
   ...
   - Why not = 却下した代替案 (例: `// ここで sort しない: 入力は呼び出し元で既に整列済み`)
3. 「コメントを消したら未来の読み手が混乱する」を判定基準にする。混乱しないなら書かない。

#### Why not（却下した代替案）の明示とルーティング基準

Why not は WHY の特殊形である。検討して却下した代替案のうち、**将来の保守者がコードを見て「もっと素直な書き方」に直したくなるもの**を、現在形の制約文としてコードと同じ場所に残す。コメントはコードと同じ場所に居続ける唯一の文書であり、この種のコメントは将来の退行的「改善」を防ぐガードレールとして最も腐りにくく価値が高い。

**ルーティング基準（判定質問）**: 「未来の読者がコードを見て、その場で誤った『改善』（却下案への差し戻し）をするリスクがあるか？」

- **Yes** → コメント（Why not）に残す — コードと共に生きるべき制約。
- **No** → commit log の `rejected(scope)` action line で十分 — 履歴で足りる（[contextual-commits.md](./contextual-commits.md) 参照）。

**両方に書く場合の書き分け**（同じ文章を両方にコピーしない）:

- **コメント側**: 「この素直な代替はこの理由で成立しない」という**現在形の制約文のみ**。経緯・番号・検討プロセスを持ち込まない（原則 2 `no_journal_comment` と整合）。
- **commit 側**: 「何を検討して、なぜ却下したか」という経緯。git/PR メタデータが受け皿。

**Example (Bad → Good)**:

```python
# Bad: 経緯ジャーナル (no_journal_comment 違反)
# 以前は再ソートしていたが、レビュー指摘で削除した
items = list(raw_items)

# Good: 現在形の制約文 (Why not)
# ここで再ソートしないこと: raw_items は呼び出し元で整列済みで、再ソートは順序保証を壊す
items = list(raw_items)
```
...
2. **ノイズ比率の暴走**: 1 つの修正で数 cycle 経るたびにジャーナルが累積し、コード:コメント比が逆転する (`state-read.sh` で 60% コメント、うち 70% がレビュー経緯)
...
番号・cycle・旧版表現を伴わなくても、コメントが「**この変更を行った理由・経緯**」（変更イベント）を語っている場合は本原則の違反とする（例: 「〜対応のため追加」「リファクタに伴い変更」「新機能 X 用に導入」）。変更動機（Why）の受け皿は commit message の action lines（例: `intent` / `decision` / `rejected` 等）であり、コードと共に残してよいのは**現在形の制約・不変条件・Why not**（原則 1）のみ。
