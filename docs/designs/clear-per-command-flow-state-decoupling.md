...
>
> **関連設計ドキュメント**:
> - [`multi-session-state.md`](./multi-session-state.md) — flow-state を per-session file（`.rite/sessions/{session_id}.flow-state`）にした構造（PR #677 で Option A 採択）。本ドキュメントの前提。
...
機構 2 が示す通り、`/clear` を継続の終端として扱うのは **意図された設計**（refs #781 / #133 / #173 / #206）。flow-state を `/clear` 越しに持続させる案は **Out of Scope**（Issue #1256）。理由:
...
| Stop hook `stop-loop-continuation.sh` + `handoff` field | コンテキスト連続前提の one-shot 継続マーカー（#1168 / #1176）。`/clear` 越しには機能し得ない |
...
| `issue:create` | 既に flow-state 非依存（#1184）。変更不要 |
...
| 判断軸 | 結論 |
|---|---|
| 安全性は保たれるか | **保たれる**。merge 可否の権威判定は Step 2 の `gh pr view`（isDraft/mergeable）。Step 1 の警告は冗長で、削除しても draft/非 mergeable は Step 2 で確実に弾かれる |
...
- continuation 機構: handoff #1168 / #1176、Stop hook `stop-loop-continuation.sh`
