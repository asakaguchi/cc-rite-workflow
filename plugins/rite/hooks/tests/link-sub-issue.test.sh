...
print_summary "$(basename "$0")" "If you change the link-sub-issue.sh argument contract (4 positional args), JSON output schema, or remove the placeholder rejection, update this test, the consuming scripts/decompose-issues.sh + sub-issue-link-handler.md, and the manual recovery command in commands/issue/create.md (link_failures 復旧手順の 4 positional args literal)."
