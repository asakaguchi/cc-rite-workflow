...
> **flow-state ring**: 本 Phase 8.2 は lint Skill invoke 直前に flow state を `phase=ingest, active=true` に patch します (v3 enum)。caller 経由時は caller の `phase=cleanup` を一時上書きし、sub-skill return 後 Mandatory After Auto-Lint Step 0/Step 1 が `phase=ingest` に re-patch → caller Mandatory After が caller の `phase=cleanup` に書き戻す ring 構造です。単独実行時は flow state 不在のため `--if-exists` で no-op (従来互換)。phase tagging により、protocol violation が起きた場合に `/rite:resume` が `phase=ingest` から正しく復帰できる (sub-step の区別は step 名で識別)。

**Pre-write** (before invoking `rite:wiki:lint --auto`): Update flow state to `phase=ingest, active=true` so the sub-skill flow is tagged with the ingest phase (v3 enum — sub-step の区別は phase 値ではなく step 名で識別する)。The `if ! cmd; then` rc capture is mandatory — silent patch failure leaves the sub-skill flow untagged, so `/rite:resume` cannot attribute the resume point to the ingest phase:
...
if ! bash {plugin_root}/hooks/flow-state.sh set \
 ...
 --if-exists; then
 echo "WARNING: flow-state.sh set (Phase 8.2 Pre-write, phase=ingest) failed — sub-skill will run unphased, so /rite:resume での phase=ingest 復帰が効かなくなる可能性. Investigate the helper exit reason in stderr above before relying on this protection again." >&2
fi
...
> **Enforcement**: protocol violation 時 (Step 0-3 を実行せず turn を閉じる) は `phase=ingest, active=true` が flow-state に残るため、`/rite:resume` が該当 step から復帰する (Stop hook も retrospective incident detector も存在しない単層設計)。
