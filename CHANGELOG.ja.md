...
- **skill return sentinel を `:completed` から `:returned-to-caller` に rename** (#1165) — sub-skill 出力 sentinel (`[lint:completed:auto]` / `[ingest:completed]` / `[cleanup:completed]` / `[create:completed:{N}]` / `[ready:completed]` / `[merge:completed]`) の literal `completed` が LLM の turn-boundary heuristic と衝突し、最終行に `completed` を出力すると orchestrator が誤って turn を閉じ、`cleanup → wiki:ingest → wiki:lint` 等のネスト経路が途中で drop される事象が複数 PR で再発した (PR #1164 cleanup、累積対策 #604 / #618 / #923 / #1144 / #1163)。terminal vocabulary を構造的に排除するため、ネスト構造を semantic に内包する `returned-to-caller` (「caller に return = caller の次 step に進む」) へ rename。各 emit site では sentinel 直前に `<!-- skill return signal: caller must continue next step -->` を併記して active disambiguation を提供する。ingest 側の parser (`wiki/ingest.md` ステップ 8.3 の `^Lint: contradictions=` regex) は 1 行目データのみに依存するため sentinel rename の影響は受けない (事前検証済み)。影響範囲: `commands/wiki/{ingest,lint}.md`、`commands/pr/{cleanup,ready,merge}.md`、`commands/issue/create.md`、`commands/sprint/execute.md`、`skills/rite-workflow/SKILL.md` + `references/{workflow-identity,sub-skill-return-protocol}.md`、`hooks/tests/create-md-invocation-symmetry.test.sh`、および `docs/SPEC.md`。後方互換: 過去 raw source / log.md / wiki history の旧 literal は `historical preservation` ポリシーに従い retroactive rename しない。新 sentinel を grep する hook 系は旧 literal を reject するため、in-flight な flow が旧 sentinel 経路上にいる場合は `/rite:resume` で復帰させる必要がある。
