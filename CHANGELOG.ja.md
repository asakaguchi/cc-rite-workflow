...
- 横断的 orchestrator return-block implicit stop regression を mitigation (#910)
  ...
  - 横断 regression test `hooks/tests/step0-immediate-bash-presence.test.sh` 追加 (24 assertion: TC-1+2+3 で **14 main pin** — `create.md` ×4 + `cleanup.md` ×3 + `ingest.md` ×7 (Issue #917 で TC-3.5/3.6/3.7 を追加) — に加えて TC-4 で **3 cross-file count assertion**、TC-5 で **5 supplementary assertion** (`create-interview.md` caller HTML literal positive pin 2 (TC-5.3 / TC-5.4) + anti-pattern revert 2 (TC-5.1 / TC-5.2) + plain-text reminder pin 1 (TC-5.5) — 注: 5 件中 caller HTML literal を target するのは 3 件のみ (TC-5.2 / TC-5.3 / TC-5.4)、TC-5.1 と TC-5.5 は plain-text 内容を target) で asymmetric weakening 補完検出、**TC-6 で 2 件の lint.md Phase 9.2 imperative recast pin** (positive pin + anti-pattern revert pin、Issue #917 cycle 5 で `commands/wiki/lint.md` Layer 3b imperative recast の false-negative 経路を閉鎖するため追加)。presence + imperative keyword pin。byte equality は既存 `caller-html-literal-symmetry.test.sh` に委譲する責務分離維持)。
