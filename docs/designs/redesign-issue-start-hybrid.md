...
3. **References 抽出**: 6-8 ファイルを `commands/issue/references/` 配下に新設
   <!-- ※ 当時の提案 list — 4 entry は後の refactoring で削除済。残り 3 entry のみ現存。 -->
...
4. **2 文 contract 標準化**: 各 Mandatory After の rationale を以下の 2 文に統一する
   ```
   > See [Flow State Scaffolding](./references/flow-state-scaffolding.md) <!-- ※ link 先 file は #1136 で削除済 — historical 参照 -->.
...
#### 新規作成
<!-- ※ 当時の提案 list — sub-skill 3 件は実際は flat workflow refactoring で覆され作成されず、reference 4 件は後の refactoring で削除済。残り 3 reference のみ現存。 -->
