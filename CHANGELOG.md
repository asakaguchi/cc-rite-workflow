...
- Completed `/rite:issue:start` redesign series (Issue #896, PR A through PR H — #905 wrap-up)
  - **Sub-skill 分割完了**: Phase 5 を 3 sub-skill chain (`rite:issue:start-execute` / `rite:issue:start-publish` / `rite:issue:start-finalize`) に分割。本体 `start.md` は 731 → 700 行に slim 化、Simplification Charter (`≤ 700` ratchet target) を達成。
  - **Test 強化** (#905):
