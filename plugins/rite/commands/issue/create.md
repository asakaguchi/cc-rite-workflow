...
# Cleanup tmpdir on any exit path (success, error, SIGINT, SIGTERM).
# 旧実装は早期 exit branch に明示 `rm -rf` を散りばめていたが、成功パス末尾と SIGINT/SIGTERM 時に
# tmpdir が残置していた (PR #1079 review: code-reviewer H-5 対応)。trap EXIT で一元化する。
