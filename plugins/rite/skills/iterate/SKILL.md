...
`/rite:review` ↔ `/rite:fix` を **指摘ゼロになるまで無限ループ** する。やることは以下のシーケンシャルなタスク列:

0. flow-state から issue_number / branch_name を復元
1. `/rite:review` を invoke
...
## ステップ 1: /rite:review を invoke

flow-state を `phase=review` に更新後、`/rite:review` を invoke:
