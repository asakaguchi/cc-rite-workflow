...
`caller-markdown-block.test.sh` TC-3 は本 form を `^if .*then$` (行末 `then`) で grep pin している (start.md 4 箇所、implement.md 1 箇所、review.md 1 箇所、resume.md 1 箇所)。 **RESUME_HINT echo** は Issue #956 で SoT 化された 4 番目の echo (詳細は §[Enforcement note (LLM 向け)](#enforcement-note-llm-向け) 参照)。
...
また `state-read.sh` 起動失敗 (上記 [Form A](#form-a-複数行-form-pre-condition--値取得-用) / [Form B](#form-b-inline-1-行-form-metrics-step-用) の `else rc=$?; ...` branch) では、`ERROR:` / `WARNING:` + `[CONTEXT] STATE_READ_FAILED=1` sentinel に続き **4 番目の echo として `RESUME_HINT:` を必ず emit する** (Issue #956 で SoT 化):

4. `RESUME_HINT: state-read.sh が異常 exit (rc=$rc) しました。ファイル不在/empty/jq parse 失敗は --default で吸収 (exit 0) されるため、本経路は helper validation 失敗 / --field 引数欠落 / invalid field name 等の caller 側引数異常で発火します。$PLUGIN_ROOT/hooks/_validate-helpers.sh と state-path-resolve.sh の存在/実行権限を確認し、必要なら /rite:resume で再開、または STATE_ROOT 配下の sessions/ を確認してください。` — operator-facing recovery hint (root cause 限定 + verify 手順 + resume path)

RESUME_HINT 本文は **Form A / Form B / 全 caller で bit-identical** とし、`caller-markdown-block.test.sh` TC-7 が SoT (本 reference) と全 caller の同一性を grep + diff で機械検証する。
...
|------|-------------|-------|------|--------------------|
| Phase 3 pre-condition | `commands/issue/start.md` | `phase` | Phase 2 完了確認 | ✅ Form A SoT mirror (Issue #956) |
| Phase 5.5.1 pre-condition | `commands/issue/start-finalize.md` (PR G2 #904 で SoT 移管) | `phase` | Phase 5.5 完了確認 | ✅ Form A SoT mirror (PR #955 で追加) |
| Phase 5.6 pre-condition | `commands/issue/start-finalize.md` (PR G2 #904 で SoT 移管) | `phase` | Phase 5.5.2 完了確認 | ✅ Form A SoT mirror (PR #955 で追加) |
| Phase 5.5.2 Metrics Step 1 | `commands/issue/references/metrics-recording.md` (Issue #901 PR E で SoT 移管) | `implementation_round` | `plan_deviation_count` 算出 (non-numeric は 0 降格、空文字列は METRICS_SKIPPED sentinel emit) | ✅ Form B SoT mirror (Issue #956) |
| Phase 5.7 parent close | `commands/issue/start-finalize.md` (PR G2 #904 で SoT 移管) | `parent_issue_number` | parent Issue の auto-close 判定 (non-numeric は 0 降格 = "no parent" 扱い) | ✅ Form A SoT mirror (PR #955 で追加) |

加えて以下 3 site も同 capture pattern を共有し、SoT mirror 義務を負う (本表 5 site canonical の外延、本 reference の "Enforcement note (LLM 向け)" 4 echo 契約に従う):

| Site | 所在ファイル | Field | 用途 | RESUME_HINT mirror |
|------|-------------|-------|------|--------------------|
| Phase 5.1.2 parent_issue_number | `commands/issue/implement.md` | `parent_issue_number` | child Issue 作業時の親 Issue 進捗更新判定 | ✅ Form A SoT mirror (Issue #956) |
| Phase 5.3.8 loop_count | `commands/pr/review.md` | `loop_count` | finding attribution の review loop count 判定 | ✅ Form A SoT mirror (Issue #956) |
| Phase 2.1 parent_issue_number_raw | `commands/resume.md` | `parent_issue_number` | resume 時の親 Issue display 整形 | ✅ Form A SoT mirror (Issue #956 で「対処:」形式から統一) |
