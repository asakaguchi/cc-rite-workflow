...
```bash
bash {plugin_root}/hooks/flow-state.sh get --field issue_number --default "" > /tmp/rite-iterate-issue-$$.txt
bash {plugin_root}/hooks/flow-state.sh get --field branch --default "" > /tmp/rite-iterate-branch-$$.txt
echo "[CONTEXT] ITERATE_ISSUE=$(cat /tmp/rite-iterate-issue-$$.txt); ITERATE_BRANCH=$(cat /tmp/rite-iterate-branch-$$.txt)"
rm -f /tmp/rite-iterate-issue-$$.txt /tmp/rite-iterate-branch-$$.txt
```
...
| `[fix:cancelled-by-user]` | **ループ終了**（ユーザーが fix.md 内 AskUserQuestion で中止選択。`/rite:resume` で再開可） |
