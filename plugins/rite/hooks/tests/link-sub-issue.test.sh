...
print_summary "$(basename "$0")" "If you change the link-sub-issue.sh argument contract (4 positional args), JSON output schema, or remove the placeholder rejection, update this test, the consuming scripts/decompose-issues.sh + scripts/backfill-sub-issues.sh + sub-issue-link-handler.md, the manual recovery command in commands/issue/create.md (link_failures 復旧手順の 4 positional args literal), the usage example in references/graphql-helpers.md, and hooks/tests/create-md-invocation-symmetry.test.sh (TC-5b が 4-positional-arg 隣接形を pin). 列挙は 4-arg contract / JSON schema の literal を持つ consumer のみ (設計ドキュメントは除く)."
