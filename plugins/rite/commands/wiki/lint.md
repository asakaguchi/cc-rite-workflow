...
if [ -n "$log_content" ]; then
 ...
 # 3 行 loud emit (WARNING + 対処 + 影響) に拡張し、Phase 6.0 log_err / Phase 6.2
...
# 注: 行番号参照は drift するため semantic 参照のみを使う (Phase 6.2 per-page loop
 ...
 *)
 ...
 # BSD variant に統一 (Phase 6.0 / 6.2 cleanup と対称化)。
...
2. **caller 継続 blockquote** (`> ⏭ MUST continue (turn を閉じない): ...`): LLM / user 両方に caller の Mandatory After 起動を促す user-visible reminder。LLM turn-boundary heuristic の fragile 誘発源 (plain-text `Lint:` 1 行で turn 終了) を抑止する。Issue #917 で imperative recast (現状報告 `継続中` → 命令形 + 否定形重ねがけ `MUST continue (turn を閉じない)`) に変更。historical: 旧 `commands/issue/create-interview.md` Return Output Format の `> ⏭ MUST continue (turn を閉じない):` と対称化していたが、現在は wiki/lint.md 単独運用の invariant。
