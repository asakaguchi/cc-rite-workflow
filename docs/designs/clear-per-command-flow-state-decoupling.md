...
> **Status**: ✅ 方針決定（調査・設計のみ。実装は後続 Issue に委ねる）
>
> **本ドキュメントの位置付け**: 本 Issue の成果物。離散実行する各 rite コマンド（`issue:create` / `pr:open` / `pr:ready` / `pr:merge`）を session-scoped flow-state への依存から切り離す設計の是非・影響範囲・最小修正案を確定する。実装 PR は本 Issue では作成せず、§7 の後続 Issue 分割案へ委ねる。
>
> **関連設計ドキュメント**:
...
flow-state は並行セッション対応のため per-session file 設計（[`multi-session-state.md`](./multi-session-state.md) の Option A）を採る。ファイル名が session_id（UUID）であり、各 hook / コマンドは自セッションの state file のみを読み書きする。
...
> **本リポでの実証**: ある PR を `/rite:pr:ready` → `/rite:pr:merge` と別ターンで実行した際に、`pr:merge` Step 1 が「Ready 化されていない可能性があります」確認と `possible stale .rite-session-id` WARNING を出した。マージ自体は `gh pr view` が `MERGEABLE`/`CLEAN` だったため安全だったが、警告 UX が「壊れている」ように見えた。本ドキュメント作成セッションでも、`flow-state.sh get` 実行時に同 WARNING が再現している（ドッグフーディングによる自己実証）。
...
| issue:create | （なし） | — | flow-state に**一切触れない**（親セッション state の誤上書きを防止する設計） | 無関係 | ✅ 該当なし |
...
| Stop hook `stop-loop-continuation.sh` + `handoff` field | コンテキスト連続前提の one-shot 継続マーカー（継続 / 終了 handoff および WIKICHAIN cleanup チェーン）。`/clear` 越しには機能し得ない |
...
- 親問題系列: Session Ownership 系列の一連の Issue（`/clear` を継続終端とする設計の出典）
- 前提設計: [`multi-session-state.md`](./multi-session-state.md)（per-session file）、[`session-ownership-flow-state.md`](./session-ownership-flow-state.md)
- continuation 機構: handoff（継続 / 終了 / WIKICHAIN チェーン）、Stop hook `stop-loop-continuation.sh`
- 実証: ある PR の `pr:ready` → `pr:merge` 別ターン実行で観測
