...
単発の子 Issue を紐付け、失敗集計を行わないケース。`parent-routing.md` sub-skill (`/rite:pr:open` 内部) の child creation path で使われていた (現在は flat 化で sub-skill 自体が削除)。現状は主に Variant B (counting) の方が `commands/issue/create.md` ステップ 5.4 で使われる。
