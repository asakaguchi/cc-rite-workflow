...
>
> 本文中の `[start:execute:completed]` / `[start:publish:completed]` / `[start:finalize:completed]` 等の sentinel literal は **pre-#1165 naming の歴史的記述** として保持する。現行 sentinel 命名規約は `plugins/rite/commands/issue/create.md` ステップ 4.4 / 5.6 の `[create:returned-to-caller:{N}]` を参照。
