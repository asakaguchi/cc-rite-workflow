...
> **セクション名について**: 歴史的経緯により名称は「外部仕様の検証結果」のままだが、本セクションは **両 Sub-Phase の検証結果** を統合して記録する (Internal Likelihood Claim の立証/降格も含む)。名称変更は caller 側 (review.md / assessment-rules.md) の同期が必要なため別 PR で対応予定。本ファイルでは `種別` 列で sub-phase 出典を明示することで名称と内容の乖離を補う。
...
> **caller 側 (review.md / assessment-rules.md) との同期**: 本ファイルは Fact-Check Phase の仕様書 (source of truth) であり、review.md `#### Fact-Check Phase` の実行手順および assessment-rules.md の `Fact-Check exclusion` / `【外部仕様検証】` 出力フォーマットは本ファイルの内容に追従する必要がある。caller 側の更新は別 PR で対応し、本ファイルと意図的に PR を分離している。
