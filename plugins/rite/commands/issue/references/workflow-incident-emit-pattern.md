...
```bash
# Step 1: emit sentinel via hook script (stderr is captured separately so emit
# failures surface as WARNING; silent suppression would hide allowlist drift /
# missing helper / bash interpretation errors from the operator).
_emit_err=$(mktemp 2>/dev/null) || _emit_err=""
sentinel_line=$(bash {plugin_root}/hooks/workflow-incident-emit.sh \
  --type {sentinel_type} \
  --details "{specific failure description}" \
  --pr-number {pr_number} 2>"${_emit_err:-/dev/null}") || {
  _emit_rc=$?
  echo "[rite] WARNING: workflow-incident-emit.sh exited non-zero (rc=$_emit_rc); sentinel may not be recorded" >&2
  [ -n "$_emit_err" ] && [ -s "$_emit_err" ] && head -3 "$_emit_err" | sed 's/^/  /' >&2
}
[ -n "$_emit_err" ] && rm -f "$_emit_err"
...
bash {plugin_root}/hooks/workflow-incident-emit.sh \
  ...
  --pr-number 0 >&2 || echo "[rite] WARNING: workflow-incident-emit.sh exited non-zero (type=manual_fallback_adopted); sentinel may not be recorded" >&2
```

`|| { echo WARNING; }` で non-blocking を保ちつつ emit 失敗を可視化する。silent `|| true` は allowlist drift / 引数 typo / helper 不在の root cause を operator triage から隠す経路を作るため使用しない。
...
bash {plugin_root}/hooks/workflow-incident-emit.sh \
  ...
  --pr-number 0 >&2 || echo "[rite] WARNING: workflow-incident-emit.sh exited non-zero (type=skill_load_failure); sentinel may not be recorded" >&2
```
...
bash {plugin_root}/hooks/workflow-incident-emit.sh \
  ...
  --pr-number {pr_number} >&2 || echo "[rite] WARNING: workflow-incident-emit.sh exited non-zero (type=manual_fallback_adopted); sentinel may not be recorded" >&2
```
...
bash {plugin_root}/hooks/workflow-incident-emit.sh \
  ...
  --pr-number {pr_number} >&2 || echo "[rite] WARNING: workflow-incident-emit.sh exited non-zero (type=skill_load_failure); sentinel may not be recorded" >&2
```
...
1. **emit failure は非 fatal**: `|| { echo "[rite] WARNING: ..." >&2; }` で workflow を halt させずに emit 失敗を可視化する。silent `|| true` は allowlist drift / helper 不在 / 引数 typo の root cause を operator triage から隠すため使用しない。
