...
>
> ```bash
> wiki_branch="{wiki_branch}" # F-03 対応: ステップ 1.1 の出力値を LLM が literal substitute する (他 5 Phase と対称化)
> case "$wiki_branch" in
> "{"*"}")
...
# 参照される本 ステップ 8.1 の信頼性を損なう。行番号は drift するため Phase 番号と semantic 名
...
|-------------|--------|------|
| `{log_path}` | ステップ 8.2 bash block が `echo "log_path=$log_path"` で stdout に emit | LLM は会話コンテキストから `log_path=...` 行を grep し、本 Phase の Edit ツール呼び出しと下記 bash block に literal substitute する |
