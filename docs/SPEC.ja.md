...
| Hook | トリガ | アクション |
|------|--------|-----------|
| `wiki-ingest-trigger.sh` | Phase 5.4.3 (review 後) / Phase 5.4.6 (fix 後) / Issue close | dev ブランチの作業ツリー `.rite/wiki/raw/{type}/` 配下に raw source ファイルを書き出す。純粋な書き出しのみで git 操作はしない |
| `wiki-query-inject.sh` | Phase 2.6 (work memory init) / Phase 5.1 (implement) / Phase 5.4.1 (review) / Phase 5.4.4 (fix) | 現在の Issue title/body に対して `/rite:wiki:query` を走らせ、マッチする heuristics を注入する。local の wiki ブランチが不在（fresh clone / 別 worktree）なら `origin/{wiki_branch}` から読む (#555) |
...
| `wiki-ingest-trigger.sh` | Phase 5.4.3（レビュー後）、Phase 5.4.6（修正後）、Issue クローズ時 | `.rite/wiki/raw/{type}/` 配下に Raw Source ファイルを書き込む（純粋なファイルライター、git 操作は行わない） |
...
| **2. Sentinel ベースの可視化** | legitimate skip (`wiki_ingest_skipped`) と commit 失敗 (`wiki_ingest_failed`) のどちらも `workflow-incident-emit.sh` 経由で workflow-incident sentinel を emit し、`start.md` Phase 5.4.4.1 が context grep で検出して `AskUserQuestion` でユーザーに提示する | `workflow-incident-emit.sh`, `start.md` Phase 5.4.4.1 |
...
完了報告 (`start.md` Phase 5.6.2) は **常に**「Wiki ingest 状況」セクションを含み、上記 signal を集約してユーザーに `/rite:issue:start` 実行中に Wiki ブランチが成長したかを明示します。AC-5 (#524) はこのセクションを全カウンタが 0 でも render することを要求します（不在自体が regression signal のため）。
