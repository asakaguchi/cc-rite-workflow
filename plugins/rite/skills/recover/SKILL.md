...
<!-- run orchestration: この「即停止して報告」に分岐した場合は STOP — 以降の「active 維持」「カーソル前進とループ」paragraph には進まない（cursor advance を経由しない）。カーソル前進を経由するのはサーキットブレーカー経路のみ（batch-run ステップ 6/8 と同じ非対称性）。 -->
