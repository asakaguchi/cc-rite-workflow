...
> **Architecture**: `/rite:issue:start` と `/rite:issue:create` は単一ファイル flat workflow として動作する。旧 sub-skill ファイル (`commands/issue/start-execute` / `start-publish` / `start-finalize` / `create-interview` / `create-register` / `create-decompose` / `parent-routing` 等) と implicit-stop ガード 3 hook (`auto-fire-step0.sh` / `verify-terminal-output.sh` / `stop-create-interview-block.sh`) は flat workflow へ統合済み。本文書中の retired コンポーネント参照箇所は移行 anchor として残置。
...
| セクション | 用途 |
|-----------|------|
| `project.type` | プロジェクトプリセット（`generic` / `webapp` / `library` / `cli` / `documentation`） |
...
| `review.*` | `loop.*`（convergence_monitoring / auto_propagation_scan / pre_commit_drift_check）、`doc_heavy.*`、`fact_check.*`（`use_context7` 含む）、`debate.*`、`security_reviewer.*`、`confidence_threshold`、`observed_likelihood_gate.*` / `fail_fast_first.*` / `separate_issue_creation.*`（すべて #506） |
| `fix.*` | `fail_fast_response`（#506）、`severity_gating`（#557 で deprecated、`false` 固定） |
...
| `workflow_incident.enabled` | Workflow incident 自動 Issue 登録（#366） |
...
| `wiki-query-inject.sh` | start.md ステップ 2.6 (work memory init) / `implement.md` Phase 5.0.W / `pr/review.md` Phase 4.0.W / `pr/fix.md` Phase 0.5.W | 現在の Issue title/body に対して `/rite:wiki:query` を走らせ、マッチする heuristics を注入する。local の wiki ブランチが不在（fresh clone / 別 worktree）なら `origin/{wiki_branch}` から読む |
...
`[CONTEXT] WORKFLOW_INCIDENT=1; type=<type>; details=<details>; (root_cause_hint=<hint>; )?iteration_id=<pr>-<epoch>` 形式の sentinel 1 行を emit する。`/rite:issue:start` ステップ 8.5 が context grep で検出し、workflow blocker を Issue として自動登録する。
...
| `manual_fallback_adopted` | ユーザーが orchestrator の `AskUserQuestion` で「手動 Edit fallback」を選択 | Orchestrator fallback prompts (start.md ステップ 5 lint:aborted, ステップ 6 pr:create-failed, `pr/fix.md` fix:error, `pr/ready.md` ready:error) |
...
Sentinel は `plugins/rite/hooks/workflow-incident-emit.sh` から emit されます。検出は `/rite:issue:start` ステップ 8.5 で context grep により行われます。

### 検出ロジック

1. **Sentinel 検出**: ステップ 8.5 で Phase 5 の各 Skill invocation 後に context grep で `[CONTEXT] WORKFLOW_INCIDENT=1` 行を検索
...
6. **Non-blocking エラーハンドリング**: Issue 作成失敗時も workflow は中断せず、incident は `workflow_incident_skipped` リストに retain され start.md ステップ 8.6 完了レポートで報告
...
| AC | 振る舞い | 実装場所 |
|----|---------|---------|
| AC-1 | Skill load failure 検出 | `start.md` ステップ 8.5 context grep + skill 内部 sentinel emit |
...
Phase 7 (review recommendation からの自動 Issue 作成) と ステップ 8.5 (Workflow Incident Detection) は **独立した codepath** であり、`create-issue-with-projects.sh` のみを共通ヘルパーとして共有します。両者は同じ `/rite:issue:start` フロー内で同時実行され、それぞれ独立した Issue を作成します。ロジックの融合はありません。

| Phase | 目的 | source フィールド |
|-------|------|------------------|
| Phase 7 | reviewer の「別 Issue として作成」推奨から Issue 作成 | `pr_review` |
| ステップ 8.5 | workflow blocker から Issue 作成 (sentinel 検出) | `workflow_incident` |
...
| フック | トリガ | アクション |
|--------|-------|-----------|
| `wiki-query-inject.sh` | start.md ステップ 2.6（work memory 初期化）、`implement.md` Phase 5.0.W（実装）、`pr/review.md` Phase 4.0.W（レビュー）、`pr/fix.md` Phase 0.5.W（修正） | 現在の Issue タイトル/本文に対して `/rite:wiki:query` を実行し、マッチした経験則を注入 |
...
| **2. Sentinel ベースの可視化** | legitimate skip (`wiki_ingest_skipped`) と commit 失敗 (`wiki_ingest_failed`) のどちらも `workflow-incident-emit.sh` 経由で workflow-incident sentinel を emit し、`start.md` ステップ 8.5 が context grep で検出して `AskUserQuestion` でユーザーに提示する | `workflow-incident-emit.sh`, `start.md` ステップ 8.5 |
...
完了報告 (`start.md` ステップ 8.6) は **常に**「Wiki ingest 状況」セクションを含み、上記 signal を集約してユーザーに `/rite:issue:start` 実行中に Wiki ブランチが成長したかを明示します。全カウンタが 0 でも render するのは、不在自体が regression signal となるため。
