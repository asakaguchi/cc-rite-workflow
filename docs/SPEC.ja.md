...
| `wiki-query-inject.sh` | start.md ステップ 2.6 (work memory init) / `implement.md` Phase 5.1 / `pr/review.md` Phase 5.4.1 / `pr/fix.md` Phase 5.4.4 | 現在の Issue title/body に対して `/rite:wiki:query` を走らせ、マッチする heuristics を注入する。local の wiki ブランチが不在（fresh clone / 別 worktree）なら `origin/{wiki_branch}` から読む |
...
| フック | トリガ | アクション |
|--------|-------|-----------|
| `wiki-query-inject.sh` | start.md ステップ 2.6（work memory 初期化）、`implement.md` Phase 5.1（実装）、`pr/review.md` Phase 5.4.1（レビュー）、`pr/fix.md` Phase 5.4.4（修正） | 現在の Issue タイトル/本文に対して `/rite:wiki:query` を実行し、マッチした経験則を注入 |
