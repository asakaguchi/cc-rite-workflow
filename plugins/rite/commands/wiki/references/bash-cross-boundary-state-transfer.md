...
- 同 ステップ 8.1 — `lint_action` 2 値 enum (`lint:clean` / `lint:warning`) の `[CONTEXT]` prefix 版 (Issue #573)。
 ...
 ため判定から除外 (Issue #563 準拠)。ステップ 8.3 の `{log_entry}` 組み立てが本 emit 値を single source
...
- `plugins/rite/commands/wiki/lint.md` ステップ 6.0 — `---skipped_refs_begin---` / `---skipped_refs_end---` で
 `ingest:skip` 済み raw の集合を伝達 (実装実体は `hooks/scripts/wiki-lint-skipped-refs.sh`)
- 同 ステップ 6.2 — `---all_source_refs_begin---` / `---all_source_refs_end---` で全 Wiki ページの
 ...
 包含) を行うために参照される (PR #564 で Pattern 2 の参照実装を拡充。実装実体は
 `hooks/scripts/wiki-lint-source-refs.sh` へ委譲済 — #1208)
...
if log_content=$(git show "${wiki_branch}:.rite/wiki/log.md" 2>"${log_err:-/dev/null}"); then
 ...
 echo " 影響: log.md 由来の skipped_refs が取得できず、stale / missing_concept 検知の false positive note を ステップ 9.1 で表示します" >&2
...
- `plugins/rite/commands/wiki/lint.md` ステップ 6.2 — `all_source_refs_read_ok` 3 値 enum (unknown / true / io_error) variant 実装。ステップ 6.0 からの派生だが、per-page で legitimate absence を吸収するため集合レベルでは `absent` 値を持たない。Pattern 3 を多対一の集合処理に適用する場合の参照 (PR #564 F-08 対応で追加。実装実体は `hooks/scripts/wiki-lint-source-refs.sh` へ委譲済 — #1208)
