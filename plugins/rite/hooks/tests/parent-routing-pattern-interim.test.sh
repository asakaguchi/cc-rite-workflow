...
# verified-review M-1 (silent-failure-hunter) 対応: 旧 inline pattern
...
#
# verified-review IMP-3 (pr-test-analyzer) 対応: 旧 `grep -A 1` 隣接 pin は cold-start branch 内
...
# verified-review M-2 (silent-failure-hunter) 対応: 旧 `2>/dev/null || true` は grep の IO エラー
...
_invocation_block_interview=$(grep -B1 -A8 'workflow-incident-emit\.sh' "$INTERVIEW_MD" 2>"${_grep_err_int:-/dev/null}")
_grep_rc_int=$?
...
_invocation_block_create=$(grep -B1 -A8 'workflow-incident-emit\.sh' "$CREATE_MD" 2>"${_grep_err_cre:-/dev/null}")
_grep_rc_cre=$?
...
# verified-review M-4 (silent-failure-hunter) 対応: 旧実装は `fail` (継続) + `else` でサブアサーション
