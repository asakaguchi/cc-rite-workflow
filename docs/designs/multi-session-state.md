...
>
> **実装済みの作業項目**:
> - Phase 1: Decision Log で Option A (per-session file) を採択
> - `flow-state-update.sh` のマルチステート対応（per-session file API + `schema_version: 2`）
> - 旧形式 `.rite-flow-state` の自動 migration 機構（atomic rename + backup）
> - lifecycle 4 hooks (`session-start` / `session-end` / `pre-compact` / `post-compact`) の自 session state 参照対応 + observability 改善
> - `pre-tool-bash-guard` / `post-tool-wm-sync` + `session-ownership.sh` helper のマルチステート整理
> - `scripts/` + `commands/` + `skills/` 配下の `.rite-flow-state` literal 直接参照を semantic API へ移行
> - 並行 2 セッション regression テスト (T-01) + Session Ownership 系 AC 再検証 (T-07) + AND 論理 8 種防御層
> - migration / atomic write / cleanup / crash resume テストスイート (T-03〜T-10)
>
...
> **Note**: `stop-guard.sh` は `develop` ブランチの直前 commit (`e2dfae0`) で撤去済みのため、本設計ドキュメントの参照対象から除外している。マルチステート構造の整合性は上記 6 hooks の自 session state 参照経路で担保する。
...
Session Ownership 機構（一連の Session Ownership 系 Issue で段階的に拡張）はこの単一ファイル前提の上に「session_id check」というガード層を追加してきたが、ガード層では root cause（並行書き込み競合）を解消できないため、漏れ経路を逐次塞ぐ N+1 回目の regression が繰り返されている。
...
本 Issue では `.rite-flow-state` を **マルチステート構造** へ再設計し、並行セッションを first-class に扱う。Session Ownership 機構（ガード層）はマルチステート構造の上に整理し直し、漏れ経路を個別 patch する戦略から脱却する。
...
- `handoff` (継続マーカーとして導入され、その後 PR 終了点での終了 handoff、さらに cleanup wiki チェーンへと段階的に拡張された。one-shot マーカー。`flow-state.sh set --handoff <value>` が writer で **毎 set デフォルトクリア** (= `--handoff` 指定時のみ key を付与、`error_count` と同じ phase-transition クリア semantics)。`flow-state.sh consume-handoff` が reader+deleter で、`Stop` hook `stop-loop-continuation.sh` が turn 終了時に呼ぶ。値は 3 種類: 継続 handoff `/rite:...` (継続 sentinel を出す `review.md` Step 8.0 / `fix.md` Step 5.1 が set → 次コマンド再注入)、終了 handoff `FINALIZE:{result}:{pr}` (終了 sentinel を出す `review.md` Step 8.0 / `fix.md` Step 5.1 / Step 1.4 cancel が set → `/rite:pr:iterate` ステップ5 完了通知を 1 回だけ強制)、チェーン handoff `WIKICHAIN:{caller}:{pr}` (`cleanup.md` ステップ 9 が `rite:wiki:ingest` invoke 前に set → cleanup → ingest → lint チェーン未完走時に残り step の継続を 1 回だけ強制。完走時はステップ 12 terminal set の default-clear で消える)。Stop hook が prefix で reason を分岐。`flow-state.sh` は任意文字列を verbatim 格納するため機構変更不要。additive かつ後方互換 (`.handoff // ""`) のため `schema_version` bump 不要)
...
1. **並行性が構造的に保証される**: lock 不要 = race window なし。過去の Session Ownership 系列で観測された「lock 取得漏れによる race」を構造的に排除できる
...
> **Status**: ✅ 実装完了 — 2026-04-26 〜 2026-05-01 にかけて 8 件の Issue として segmented で land

| 項目 | 値 |
|------|-----|
| 採択方式（再掲） | Option A: per-session file (`.rite/sessions/{session_id}.flow-state`) |
| Phase 2 実装期間 | 2026-04-26 〜 2026-05-01（5 営業日） |
| 関連実装 | 冒頭「実装済みの作業項目」の 8 件（flow-state API のマルチステート対応、自動 migration、lifecycle hooks の自 session 参照、semantic API 移行、regression / テストスイート）に対応 |
| AC-1〜AC-12 最終 pass 状況 | T-01〜T-10 + T-12 すべて green。T-11 (Option B 用 lock 自動解放) は Option A 採択により対象外 |
...
| `loop_count` field | reader 経路 (`pre-compact.sh` / `post-compact.sh` / `session-start.sh` / `work-memory-update.sh`) は存在するが、`flow-state.sh` 内に **`loop_count` への writer は実装されていない** (`grep -n loop_count plugins/rite/hooks/flow-state.sh` → 0 hits)。`work-memory-update.sh` の `loop_count` 増分は work-memory ドキュメント側のフィールドを更新するもので flow-state の `loop_count` には書き戻されない。Phase 1 設計時 (本ドキュメント上記 Option A 「実行時に追加される optional フィールド」note) と整合する状態のまま、reader-only legacy field として schema slot を維持し将来 writer 実装に備える |
| `error_count` field | 現行 production code に **incrementer 経路は存在しない**（`stop-guard.sh` は撤去済み）。`flow-state.sh set` が phase transition 時に `0` リセット writer として書込（`--preserve-error-count` 指定時は既存値を保持）。incrementer 不在の half-legacy field として schema は維持し、将来再導入される可能性に備える |
| `--legacy-mode` flag | `migrate-flow-state.sh` の auto-migration 機構が `schema_version` キー欠落 or `< 2` の旧形式を起動時に自動検出 → atomic rename で新形式に変換するため、`--legacy-mode` flag は導入されなかった。代わりに `rite-config.yml` の `flow_state.schema_version: 1` で強制 fallback 可能（adapter pattern）だった（この fallback は後続作業で撤去済み — 下記「Rollback 戦略（撤去済み）」節参照） |
...
| 新規 file 命名と既存 glob の collision | migration 実装の review cycle で CRITICAL を実測: 旧 `.rite-flow-state.??????*` glob と新 `.rite-flow-state.legacy.<timestamp>` 命名が偶発的に一致し、`-mmin +1` 経過後に silent 削除される regression を実測。`-not -name` 例外 + 命名 token の test 化 + negative counter-test で defense-in-depth 化（Wiki 経験則として永続化） |
| AND 論理 8 種防御層 | T-07 で再検証済み。`.rite-flow-state.active=true` 前提条件への依存は新形式（per-session file の存在チェック）でも維持されており、過去 9 件の Session Ownership 系列 Issue で導入した防御層は新形式上でも全て稼働 |
| 並行性の構造保証 | T-01 で 2 セッション同時起動を実測。lock 不要で race window が観測されないことを確認。Session Ownership 系列で繰り返された race window の root cause が解消された |

**Decision Log の grep evidence 化（Decision Log を grep で連続参照可能にして N+1 回目の同種判断時の再評価コストを下げる preventive measure）**:
...
当初は `rite-config.yml` に `flow_state.schema_version` を導入し（初期値 `2`）、`schema_version: 1` を明示すると旧形式（single-file `.rite-flow-state`）で動作する fallback path を adapter pattern で保持していた。

この dual-logic は後続作業で撤去済み。flow-state は常に per-session（`.rite/sessions/{session_id}.flow-state`）で動作し、`flow_state.schema_version` による形式選択分岐は存在しない。明示的な `schema_version: 1` は無視され、`session-start.sh` が session start ごとに 1 回（キー削除まで毎回）deprecation 警告を出して dead key の削除を促す。残存する `.rite-flow-state` single-file は `flow-state.sh migrate` 経路で per-session/v3 へ吸収される。
...
| AC-7 | T-07 | Session Ownership 系列の AC 再検証 |
...
- 後続設計: [multi-session-worktree.md](./multi-session-worktree.md) — 本設計が分離した「状態」レイヤ（per-session flow-state）の上に「git 作業ツリー / カレントブランチ」レイヤの分離を完成させる Epic（セッション別 Git worktree + EnterWorktree 自動入場）。本設計の `session_id` ベース構造を claim liveness 判定がそのまま再利用する
- 親系列: Session Ownership 親 Issue — 本設計で root cause を解消し、ガード層から構造へ昇格
- 過去対策: 漏れ経路を逐次塞いだ N+1 patch 履歴（Session Ownership 系列の一連の Issue）
