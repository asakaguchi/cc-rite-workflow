...
- 18 orphan hook tests removed in #1079 (4 tests directly named after deleted hooks + 13 tests structurally tied to the deleted sub-skill chain + `start-md-charter.test.sh` which referenced deleted `start-execute.md` / `start-publish.md` / `start-finalize.md` and silently passed only because `count_aggregated` skipped non-existent files). Also removed `tests/fixtures/start-md/` (only consumed by the charter test). New tests for the flat workflow are added inline (`incident-emit-callsite-coverage`, `start-md-sentinel-coverage`, `phase-transition-whitelist`, `issue-body-safe-update`, `link-sub-issue`, `post-compact-classification-regex`).
