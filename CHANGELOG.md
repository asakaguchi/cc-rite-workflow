...
- **BREAKING: the reviewer read-only guarantee is re-layered — `pre-tool-bash-guard.sh` no longer machine-blocks working-tree git verbs, keeping only the `.git`-write machine gate** — the static verb denylist (sub-blocks (A)–(G): `checkout`/`reset`/`add`/`commit`/`branch`/`stash`/`fetch` flags/`worktree` sub-actions and 20+ more, plus the git global-flag normalization and the worktree-add argument scan) is removed from the PreToolUse hook (922 → ~440 lines), ending the recurring bypass-patch churn (11 hardening commits in 3 months) and structurally eliminating its false-positive class. Working-tree mutations are visible and recoverable via `git status`, so their guarantee is now Layer 1 (the READ-ONLY contract in `_reviewer-base.md`, unchanged) + Layer 3 (`post-review-state-verify.sh` branch/stash/branch-list/worktree drift detection after each review, unchanged). The machine gate keeps only what those layers cannot cover: writes into a `.git` directory (invisible to `git status`, irreversible, RCE-grade — sub-block (H)), the shell-wrapper block (`eval`/`sh -c`/… would trivially hide a `.git` write), and the oversized-command length guard (timeout→fail-open bypass prevention), all still fail-closed. Deny pattern names change accordingly: `reviewer-state-mutating-git` no longer exists; the surviving gates emit `reviewer-gitdir-write` / `reviewer-shell-wrapper` / `reviewer-oversized-command`. If a regression appears, individual verbs can be re-added without reviving the whole harness. (#1879)
