...
> **Reference**: Canonical Sub-skill Return contract は [`skills/rite-workflow/SKILL.md`](../../skills/rite-workflow/SKILL.md) "Sub-skill Return — Flat Workflow" セクションを参照。**Mandatory After Wiki Ingest は本 ingest.md ファイル固有の contract** (start.md / create.md の flat workflow が削除した「sub-skill chain return」rule とは独立)。本 Phase 8 (`rite:wiki:lint --auto` Skill invocation) でも同種ルールを適用: DO NOT end your response after the lint sub-skill returns, DO NOT re-invoke the completed skill, and IMMEDIATELY proceed to the 🚨 Mandatory After Auto-Lint section in the **same response turn**.

> **Defense layers in effect**: (1) the orchestrator prompt contract in `commands/pr/cleanup.md` and this file telling the LLM not to stop; (2) caller HTML continuation hint + sub-skill HTML continuation comment immediately before each result-pattern emit; (3) retrospective detection — if the LLM stops anyway, `start.md` ステップ 8.5 greps the next session's context for `manual_fallback_adopted` and auto-registers a tracking Issue. There is no Stop hook intercepting `end_turn`.

### Pre-check list (mandatory before ending any response turn)

**Enforcement coupling**: `ingest.md` は flow-state.sh を直接呼ばない設計 (Phase 9.1 「flow-state ownership」設計判断) のため、単独実行時は flow-state による phase tagging が無い。caller (`pr/cleanup.md` Phase 4.W) からの sub-skill invoke 経由時のみ caller 側 `phase=cleanup` (v3 enum) が flow-state を支配し、protocol violation 発生時に retrospective に `manual_fallback_adopted` workflow_incident sentinel が emit されて ステップ 8.5 (start.md 配下) で post-hoc 検出される (AC-7)。本 Pre-check list は両経路で共通に適用するが、retrospective enforcement の主たる発火経路は caller 経由のみである。
...
**Defense-in-depth — flow-state ring** (Issue #618 / #917, supersedes the former #608 follow-up YAGNI stance; v3 enum 反映済 — PR #1089 review C3): `ingest.md` は Phase 8.2 直前 (Pre-write、`phase=ingest, active=true`) / 🚨 Mandatory After Auto-Lint Step 0 + Step 1 (idempotent 二重 patch、`phase=ingest, active=true` — Step 0 が FIRST patch、Step 1 が idempotent retry、Issue #917 で 5 site canonical 対称化) / Phase 9.1 Step 3 (terminal patch、`phase=ingest, active=false`) の **3 logical patch site (4 physical bash call)** で `flow-state.sh` を呼ぶ。v3 enum では全 callsite が `--phase "ingest"` を書き込み、sub-step の logical 区別 (Pre-write / Step 0 / Step 1 / Step 3) は phase 値ではなく document 内の step 名で識別する。caller 経由時は caller の `phase=cleanup` を一時的に `phase=ingest` に上書きし、sub-skill return 後 caller の Mandatory After (例: cleanup.md `🚨 Mandatory After Wiki Ingest`) が caller の `phase=cleanup` に書き戻す ring 構造で完遂する。単独実行 (`/rite:wiki:ingest`) 時は flow-state が不在 (caller 未起動) のため `--if-exists` フラグで no-op となり、従来挙動と互換。本反転の目的は Issue #621 (cleanup→ingest) と同型の Mode B 症状 (ingest→lint return 後の implicit stop) を多層防御で防ぐこと (Issue #618 AC-2/AC-3/AC-4)。
...
if [ "$branch_strategy" = "same_branch" ]; then
 ...
 # BSD variant に統一 (lint.md Phase 6.0 / 6.2 / 8.3 と対称化)。
 # bash-trap-patterns.md の『BSD/macOS rm の rm -f "" 対応 (空引数ガード variant)』規範に準拠。
...
> **flow-state ring**: 本 Phase 8.2 は lint Skill invoke 直前に flow state を `phase=ingest, active=true` に patch します (v3 enum)。caller 経由時は caller の `phase=cleanup` を一時上書きし、sub-skill return 後 Mandatory After Auto-Lint Step 0/Step 1 が `phase=ingest` に re-patch → caller Mandatory After が caller の `phase=cleanup` に書き戻す ring 構造です。単独実行時は flow state 不在のため `--if-exists` で no-op (従来互換)。phase tagging により、protocol violation が起きた場合に retrospective workflow_incident detection が `phase=ingest` 由来の incident として correlate できる (sub-step の区別は step 名で識別)。

**Pre-write** (before invoking `rite:wiki:lint --auto`): Update flow state to `phase=ingest, active=true` so the sub-skill flow is tagged with the ingest phase (v3 enum — sub-step の区別は phase 値ではなく step 名で識別する)。The `if ! cmd; then` rc capture is mandatory — silent patch failure leaves the sub-skill flow untagged, so the retrospective `manual_fallback_adopted` correlation in `start.md` ステップ 8.5 has no phase to attribute the incident to:

```bash
if ! bash {plugin_root}/hooks/flow-state.sh set \
 --phase "ingest" --active true \
 --next "After rite:wiki:lint --auto returns: run 🚨 Mandatory After Auto-Lint (Step 0 + Step 1: idempotent re-patch of phase=ingest) → Phase 8.3-8.5 → Phase 9 (Completion Report + caller continuation HTML comment + <!-- [ingest:completed] --> sentinel as absolute last line) in the SAME response turn. Do NOT stop." \
 --if-exists; then
 echo "WARNING: flow-state.sh set (Phase 8.2 Pre-write, phase=ingest) failed — sub-skill will run unphased, so retrospective workflow-incident correlation against phase=ingest will not fire. Investigate the helper exit reason in stderr above before relying on this protection again." >&2
fi
...
> **Enforcement**: protocol violation 時 (Step 0-3 を実行せず turn を閉じる) は次回 turn の ステップ 8.5 (start.md 配下) で retrospective に検出される — orchestrator が context grep で `manual_fallback_adopted` sentinel を拾い、tracking Issue を自動登録する。

> **Anti-pattern 警告**: Lint Skill return 直後に「Lint: 0 件の問題」のような短いサマリ 1 行で turn を閉じてはならない。これは sub-skill return tag を turn 境界として誤認する LLM turn-boundary heuristic の誤発火による既知パターンであり、Phase 9 完了レポート + caller 継続 HTML コメント + sentinel HTML コメントが欠落する。

<!-- >>> DRIFT-CHECK ANCHOR: 🚨 Mandatory After Auto-Lint ring <<<
 Self-check 構造は cleanup.md `🚨 Mandatory After Wiki Ingest` Self-check と同型 (同じ
 Yes/No 分岐 + 同じ terminal 重複呼び出し防止 rationale)。Step 0/1 二重 patch design は
 cleanup.md と byte-equal 相当の対称構造で、transient failure 下でも Step 0 / Step 1 の
 いずれかが成功することを保証する。
 v3 enum (PR #1080 / Phase E) 以降は phase-transition-whitelist.sh は retired、phase 遷移の
 validation は flow-state.sh の PHASE_ENUM_V3 (13 値) のみで行う。いずれかを変更する場合、
 以下 site を同時確認:
 - plugins/rite/commands/pr/cleanup.md `🚨 Mandatory After Wiki Ingest` Step 0 / Step 1
 <<< END DRIFT-CHECK ANCHOR: 🚨 Mandatory After Auto-Lint ring >>> -->

**Self-check and branching**:

1. **Has `<!-- [ingest:completed] -->` been output as the absolute last line of the response?**
 - **Yes** — **terminal 到達後の重複呼び出し防止のための例外経路 (re-entry 防御)**。本 Self-check の evaluation timing (lint Skill return 直後、Phase 8.3 開始前) では **No 分岐のみが真**となり、Yes 分岐は response が既に sentinel で閉じられた後に何らかの理由で本 section が**再 evaluate** された場合のみ到達する (通常フローでは到達不能)。再 evaluate 時の flow state は terminal state (`phase=ingest, active=false` — 単独実行時) / caller の `phase=cleanup, active=true` (caller 経由時に caller Mandatory After が書き戻した後) / caller terminal state (`phase=cleanup, active=false`) のいずれか (v3 enum)。Step 0-3 below MUST be skipped. 理由: terminal 後に Step 0 / Step 1 の `patch --phase ingest --active true --if-exists` を再実行すると active=false の terminal state を上書きし flow state の terminal 性を破壊する (cleanup.md `🚨 Mandatory After Wiki Ingest` Self-check と同じ rationale)。
 - **No** — Phase 8.3 / 8.4 / 8.5 / 9 have NOT been completed yet (`phase=ingest, active=true` の非 terminal 状態、本 Self-check の正常 evaluation timing)。Steps 0-3 below are **critical** — execute immediately.
...
**Step 1**: Re-affirm flow state as `phase=ingest, active=true` (idempotent re-patch — v3 enum では引き続き `phase=ingest` を維持し、Step 0 が既に書いた timestamp / `next_action` を refresh する)。2 重 patch design は transient failure 下でも Step 0 / Step 1 のいずれかが成功することを保証し、同時失敗時のみ `[CONTEXT] STEP_1_PATCH_FAILED=1` を retained flag として残す。`--preserve-error-count` は Step 0 と対称に付与 (`.error_count` は現状 half-legacy schema slot で production reader 不在だが、future reader 再導入時の累積カウントを失わないよう保持する):
...
>
> **bash tool 実行 note**: 本規約の禁止対象は **assistant response の markdown text content** への追加行のみ。Step 3 terminal patch の bash tool 呼び出しは Claude Code の実行モデル上 response markdown text には content を追加しない (bash output は Bash tool result として別枠表示) ため、Step 3 を #3 sentinel 出力後に実行することは本規約の対象外である。**Step 3 bash 実行は Output ordering の #1/#2/#3 のいずれにも属さない meta-step** (下記「設計メモ」参照)。Step 3 を「#2/#3 間への挿入」と誤解して Step 3 を廃止 or #1 前に移動する修正は silent regression 誘発のため禁止 (Option A を仮採用すると Step 3 execution timing が Phase 9 本体処理と分離できず、terminal state 保証のタイミング contract が壊れる)。
>
...
> **設計判断 — flow-state ring** (Issue #618 / #917, supersedes the former YAGNI stance; v3 enum 反映済 — PR #1089 review C3): `ingest.md` は Phase 8.2 Pre-write (`phase=ingest, active=true`)、🚨 Mandatory After Auto-Lint Step 0 + Step 1 (idempotent 二重 patch、`phase=ingest, active=true` — Step 0 が FIRST patch、Step 1 が idempotent retry、Issue #917 で 5 site canonical 対称化)、Phase 9.1 Step 3 (terminal: `phase=ingest, active=false`) の 3 logical patch site (4 physical bash call) で flow-state.sh を呼ぶ。v3 enum では全 4 callsite が `--phase "ingest"` を書き込み、sub-step の logical 区別は phase 値ではなく document 内の step 名で識別する。caller 経由時は caller の `phase=cleanup` を一時上書きし、sub-skill return 後 caller Mandatory After (例: cleanup.md `🚨 Mandatory After Wiki Ingest`) が caller の `phase=cleanup` に書き戻す ring 構造で完遂する。単独実行時は `--if-exists` により flow-state 不在なら no-op (従来互換)。本設計反転は Issue #618 (ingest→lint return 後の implicit stop を多層防御で防ぐ) の AC-2/AC-3 要件に対応。
...
<!-- 設計メモ (非レンダリング注釈):
 ...
 (a) Phase 9.1 Step 3 の prose 見出し「Step 3 (terminal patch)」
...
> **Informational — 実 caller の現状**: 現時点で本 skill を Skill ツール経由で invoke する caller は `pr/cleanup.md` Phase 4.W のみ (`phase=cleanup` で active flow-state を持つ — v3 enum)。`pr/review.md` / `pr/fix.md` / `issue/close.md` の Wiki 関連 Phase は Issue #547 以降 `wiki-ingest-trigger.sh` + `wiki-ingest-commit.sh` の単一プロセス設計に移行済みで、Skill: `rite:wiki:ingest` を invoke しないため、これらの phase で本 sentinel を消費するパスは存在しない。単独実行時に caller 継続コメントを出力しても無害 (該当する caller がいないため grep 結果が利用されないだけ) のため、判定 logic を持たず常に出力する設計を採用する。
...
**Step 3 (terminal patch)**: Step 2 の sentinel (= Output ordering #3、assistant response **text** の absolute last line) を出力した**後**に、flow state を terminal state (`phase=ingest, active=false` — v3 enum) に patch する。本 step が **Step 2 の絶対最終行性を壊さない** 理由: Claude Code の実行モデル上、**bash tool の stdout/stderr は assistant response の markdown text content とは別チャンネル**であり、response 最終行の判定は markdown content (Step 1-2 の text output) に対して行われるため、Step 2 sentinel 出力後に bash block を実行しても response の絶対最終行は sentinel のまま保たれる (bash output は conversation 上 Bash tool result として別枠表示されるが response text の一部ではない)。したがって実行順序は document 記載順序と一致: Step 1 (caller 継続 HTML コメント出力) → Step 2 (sentinel 出力) → Step 3 (bash tool 実行)。本 Step 3 の bash 実行は MUST NOT #621 reinforce の「#2/#3 間への recap 挿入禁止」対象外 — 禁止されるのは #2 と #3 の間の response text 追加行であり、bash tool 呼び出しは response text content に相当しないため (同規約の下方 "bash tool 実行 note" 参照)。caller 経由時は直後に caller Mandatory After が caller の `phase=cleanup, active=true` に書き戻すため、本 deactivate は caller による書き戻しと衝突しない (v3 enum では phase 値が同じ `ingest` のままで active=false → caller phase + active=true へ patch される):

```bash
if ! bash {plugin_root}/hooks/flow-state.sh set \
 --phase "ingest" --active false \
 --next "rite:wiki:ingest completed. Single-session run terminated; caller (if any) MUST proceed to its own Mandatory After section to write back its own phase (e.g. phase=cleanup, active=true for the cleanup caller). Do NOT stop only if a caller is pending continuation." \
 --if-exists; then
 echo "WARNING: flow-state.sh set (Phase 9.1 Step 3 terminal, phase=ingest active=false) failed — flow-state may still report active=true. Caller Mandatory After will still be able to write back its own phase via --if-exists. Investigate the helper exit reason in stderr above." >&2
fi
...
# で再活性化し caller の `phase=cleanup` に書き戻すため、本 deactivate は transient state である
# (v3 enum では phase 値が同一であっても active 切り替えにより state が transition する)。
```
