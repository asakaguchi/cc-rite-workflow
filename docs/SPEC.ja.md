...
`/rite:issue:start` の一気通貫実行中にステップが失敗または skip された場合 (Skill ロード失敗 / hook 異常終了 / Wiki ingest skip・失敗 / `.gitignore` drift 等)、該当する script や hook はプレーンな `WARNING` / `ERROR` 行を **stderr** に出力します。orchestrator LLM はこれを会話コンテキストで surface し、ユーザーは `/rite:resume` で該当ステップを再実行して対応します。

> **History (PR 2b, #1088)**: 以前の設計 (#366) ではこれらを「workflow incident」として自動検出していました。各 failure path が専用の `workflow-incident-emit.sh` hook 経由で `[CONTEXT] WORKFLOW_INCIDENT=1; ...` sentinel を emit し、`/rite:issue:start` ステップ 8.5 が会話コンテキストを grep して blocker を Todo Issue として自動登録する仕組みでした (`AskUserQuestion` 確認、session 内 dedupe、`workflow_incident.enabled` opt-out)。emit hook・ステップ 8.5 検出ロジック・`workflow_incident:` config key・sentinel フォーマットを含む機構全体は、上記のプレーン stderr による単層設計に置き換えるため撤去されました。失敗は可視化されますが自動登録はされず、Issue を起票するかはユーザーが判断します。

### Phase 7 (review recommendation からの Issue 作成)

Phase 7 — reviewer の「別 Issue として作成」推奨から自動 Issue を作成する経路 (`source: pr_review`) — は上記の surfacing 経路とは無関係です。`/rite:issue:start` 内で実行され、ユーザー承認された reviewer 推奨に対して `plugins/rite/scripts/create-issue-with-projects.sh` を直接呼び出します。
...
完了報告 (`start.md` ステップ 8.5) は **常に**「Wiki ingest 状況」セクションを含み、上記 signal を集約してユーザーに `/rite:issue:start` 実行中に Wiki ブランチが成長したかを明示します。全カウンタが 0 でも render するのは、不在自体が regression signal となるため。
