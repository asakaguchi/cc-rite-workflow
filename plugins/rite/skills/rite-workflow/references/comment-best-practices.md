...
番号・cycle・旧版表現を伴わなくても、コメントが「**この変更を行った理由・経緯**」（変更イベント）を語っている場合は本原則の違反とする（例: 「〜対応のため追加」「リファクタに伴い変更」「新機能 X 用に導入」）。変更動機（Why）の受け皿は commit message の action lines（`intent` / `decision` / `rejected`）であり、コードと共に残してよいのは**現在形の制約・不変条件・Why not**（原則 1）のみ。
