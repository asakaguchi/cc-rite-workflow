...
| Input | 解決 |
|-------|------|
| 番号トークン `#N` | まず `gh pr view N` を試す。成功すれば `{pr_number}=N` とし、PR 本文の `Closes/Fixes/Resolves #M`（無ければ head ブランチ名の `issue-<番号>`）から `{issue_number}` を導出。失敗すれば N を Issue とみなして `{issue_number}=N` とし、`gh pr list --state all --head "*issue-N*" --json number,title,body` で関連 PR があれば `{pr_number}` に充てる（無ければ Issue のみで進める） |
| (なし) | 現在ブランチ名を `git rev-parse --abbrev-ref HEAD` で取得し、`rite-config.yml` の `branch.pattern`（既定 `{type}/issue-{number}-{slug}`）から `{issue_number}` を導出 → `gh pr list --head <branch> --json number,title,body` で `{pr_number}` を特定 |
