...
3. **Historical (retired in PR #675 / #1079)**: 旧 `stop-guard.sh` hook が flow state の `.active=true` かつ継続必須 phase (例: `create_post_interview` / `create_delegation` / `phase5_post_review` 等) で stop を block し、`workflow_incident sentinel` + WORKFLOW_HINT を stderr 経由で feed back する設計だった。現在は phase-transition-whitelist + 各 orchestrator の `🚨 Mandatory After ...` セクションのテキスト契約 + `pre-tool-bash-guard.sh` の事前ガードに役割を分散している。
