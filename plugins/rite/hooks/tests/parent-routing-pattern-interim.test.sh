...
# を一括削除 / `--preserve-error-count` に書き換え) を検出する。実測 6 occurrences (4 invocation 内で
# backslash 続行により `--if-exists` 単独行が複数 count される rationale)。最低 3 を要求して将来の正当な
# refactor (1 site のみ撤廃等) でも catch する。
