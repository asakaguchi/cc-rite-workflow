...
- **`multi_session.enabled` のデフォルトを `false` → `true` に変更 — マルチセッション worktree 分離をデフォルト ON 化** (#1391) — 明示的な**設計思想の転換**である: Worktree Mode（Epic #1360、#1381–#1388 で hardening）は「opt-in / default off / 無効時は挙動変化ゼロ」を設計方針として構築・文書化されてきたが、実運用に耐える段階に達したため、`/rite:init` の config テンプレートが `multi_session.enabled: true` を配布するようにした。これにより新規プロジェクトはセッション別 Git worktree 分離（1 ターミナル = 1 Issue、`.rite/worktrees/issue-{N}`）を自動で得る。並行セッション間の作業ツリー混線・ブランチ取り違えを構造的に防ぐ。**後方互換は維持**する: `pr:open` の config パーサは `multi_session:` ブロックが完全に欠落している場合は `false` に fallback するため、機能導入前から存在する既存プロジェクトの config は影響を受けない — 新デフォルトが効くのは新規生成された `/rite:init` config のみ。`enabled: false` を明示設定すれば #1391 以前の単一セッション挙動（byte-identical）に戻る。EnterWorktree の opt-in ガード（「ユーザー / プロジェクト指示で明示された場合のみ」）は default-on でも成立する: コミット済み `rite-config.yml` の `multi_session.enabled: true` は、その値がテンプレートのデフォルト由来かユーザーの明示編集由来かに依らず、それ自体がプロジェクト指示として機能するため。ディスクコスト（各セッション worktree はフル作業ツリーの clone、`node_modules`/`venv`/build キャッシュは非共有）の注意書きはテンプレコメントと on-demand FAQ に残す。影響ファイル: `rite-config.yml`（dogfood リポジトリ）、`plugins/rite/templates/config/rite-config.yml`（ブロックを `--- Advanced ---` マーカー上へ移動し `/rite:init` 新規生成で emit されるようにした）、`commands/pr/open.md`（ステップ 1.4 / 2.2 / 2.3-W）、`commands/getting-started.md`（Phase 4.5 FAQ）、`commands/workflow.md`（Tips ブロック）、`commands/init.md`（Step 4 `--upgrade` 分類 — 後方互換維持のため `multi_session` を back-add から除外）、`references/git-worktree-patterns.md`、`docs/CONFIGURATION.md`、`docs/SPEC.md`、`docs/designs/multi-session-worktree.md`。`.gitignore` 要件（`.rite/worktrees/`）は既に `/rite:init` が付与し、default-on 下でも `gitignore-health-check.sh` が警告を出さないことを検証済み。
- **skill return sentinel を `:completed` から `:returned-to-caller` に rename** (#1165) — sub-skill 出力 sentinel (`[lint:completed:auto]` / `[ingest:completed]` / `[cleanup:completed]` / `[create:completed:{N}]` / `[ready:completed]` / `[merge:completed]`) の literal `completed` が LLM の turn-boundary heuristic と衝突し、最終行に `completed` を出力すると orchestrator が誤って turn を閉じ、`cleanup → wiki:ingest → wiki:lint` 等のネスト経路が途中で drop される事象が複数 PR で再発した (PR #1164 cleanup、累積対策 #604 / #618 / #923 / #1144 / #1163)。terminal vocabulary を構造的に排除するため、ネスト構造を semantic に内包する `returned-to-caller` (「caller に return = caller の次 step に進む」) へ rename。各 emit site では sentinel 直前に `<!-- skill return signal: caller must continue next step -->` を併記して active disambiguation を提供する。さらに、sentinel literal 自体も全 emit site で HTML コメント形式 (`<!-- [skill:returned-to-caller] -->`) に統一し、user-visible な末端が bare bracket sentinel になる経路を排除した — disambiguator と sentinel の両方が全 emit site (`create.md` / `cleanup.md` / `ingest.md` / `ready.md` / `merge.md` / `wiki/lint.md`) で HTML コメント化されている。ingest 側の parser (`wiki/ingest.md` ステップ 8.3 の `^Lint: contradictions=` regex) は 1 行目データのみに依存するため sentinel rename の影響は受けない (事前検証済み)。関連 drift の再発を機械的に予防するため、meta-test を 2 件追加した: (1) `hooks/tests/wiki-lint-ingest-contract-sync.test.sh` は `lint.md` ↔ `ingest.md` の line-count contract 同期を assert (Asymmetric Fix Transcription anti-pattern 対策、TC-1b で `lint.md` ステップ 9.2 の SoT 単一性を `grep -c = 1` で機械検証)、(2) `hooks/tests/sentinel-disambiguator-adjacency.test.sh` は `create.md` 以外の 5 producer (`cleanup` / `merge` / `ready` / `wiki/lint` / `wiki/ingest`) 横断で disambiguator/sentinel の count parity を assert (既存 `create-md-invocation-symmetry.test.sh` TC-7a/7b の adjacency check と相補的に、全 producer で silent marker strip を機械検出可能化)。影響範囲: `commands/wiki/{ingest,lint}.md`、`commands/pr/{cleanup,ready,merge}.md`、`commands/issue/{create.md,references/bulk-create-pattern.md}`、`commands/sprint/execute.md`、`skills/rite-workflow/SKILL.md` + `references/{workflow-identity,sub-skill-return-protocol}.md`、`hooks/tests/{create-md-invocation-symmetry,wiki-lint-ingest-contract-sync,sentinel-disambiguator-adjacency}.test.sh`、`hooks/scripts/tests/test-hardcoded-line-number-check.sh`、および `docs/SPEC.md`、`docs/anti-patterns/cleanup-wiki-ingest-turn-boundary.md`、`docs/tests/rite-issue-create-e2e-smoke.test.md`。後方互換: 過去 raw source / log.md / wiki history の旧 literal は `historical preservation` ポリシーに従い retroactive rename しない。旧 literal 残存の grep 検出は手動 smoke 手順 (`docs/tests/rite-issue-create-e2e-smoke.test.md` シナリオ 3) が担い、`hooks/tests/` のメタテストは新 `returned-to-caller` sentinel の存在と disambiguator parity を assert する (旧 literal 残存の自動検出は行わない)。runtime hook が flow を block するわけではない。in-flight な flow が旧 sentinel 経路上にいる場合は、新 command / SKILL 定義が旧 literal を continuation trigger として認識しなくなる (orchestrator の sentinel 認識が match しない) ため、`/rite:resume` で復帰させる必要がある。
...
- 新規 sentinel: `[merge:returned-to-caller]` / `[merge:not-ready]` / `[merge:error]` (`/rite:pr:merge` 専用、#1165 で `[merge:completed]` から rename)
- **i18n 機構の全廃 (日本語直書き化)** (#1117) — `{i18n:KEY}` プレースホルダー機構を廃止。残り 10 command / sub-skill ファイル 364 箇所のプレースホルダーを ja カタログ値の日本語直書きに解決し、runtime i18n 解決への依存が消えた。start / create / pr 系は先行 PR で移行済みで、#1117 で完了 (全 command が i18n-free)。`plugins/rite/i18n/` ディレクトリ (ja/en split message ファイル + `i18n-usage.md`) はディスクからも完全削除済 (commit `d3a105f1`)。plugin ソースツリーに language file structure は一切残っていない
...
### 変更 (pre-#1136 履歴 — コマンド分解により superseded)
...
- 旧 sub-skill chain アーキテクチャに紐付いた orphan hook test 17 件を #1079 で削除 (削除済み hook 名を直接持つ 3 件 + アーキテクチャ依存の 13 件 + 削除済み `start-execute.md` / `start-publish.md` / `start-finalize.md` を参照していた `start-md-charter.test.sh`。後者は `count_aggregated` が不在ファイルを silent skip するため見かけ上 PASS していたが、aggregated 下限 assert が 0 計上で機能停止していた)。同テストのみが消費していた `tests/fixtures/start-md/` も削除。flat workflow 向けの新規テストは本 PR 内で追加済み (`incident-emit-callsite-coverage`、`start-md-sentinel-coverage`、`phase-transition-whitelist`、`issue-body-safe-update`、`link-sub-issue`、`post-compact-classification-regex`、`create-md-invocation-symmetry`、`resume-md-phase-mapping`)。注: `step0-imperative-phrasing-cleanup-ingest` は #1079 で追加されたが、本テストが pin していた defense 層と共に #1149 (Issue #1144) で構造的 caller chain 解消の一環として retire された (test 自体と pin literals を同時削除)。
...
- `/rite:issue:create` の implicit stop 対策として新規 Stop event hook `hooks/stop-create-interview-block.sh` を追加 (#920、累積系列 #910 / #917 / #634 / #651 / #622 / #552 / #687) — **#1079 で release 前に削除**
  ...
  - 新規 `hooks/tests/stop-create-interview-block.test.sh` (9 TC、24 assertion) で動作を pin: gate 一致 → exit 2 + ACTION + sentinel / phase 不一致・active=false・pr_number≠0・stop_hook_active・state file 不在・non-Stop event → exit 0 / `workflow_incident.enabled=false` → block 尊重・sentinel emit 抑止 / `rite-config.yml` 存在で `workflow_incident:` セクション不在 → block + sentinel emit (default-on 尊重、strict-mode + ERR-trap 互換性の regression guard)。
...
- `/rite:lint` に `gh issue create` 直接呼び出しの guard を追加 (#958)
  ...
  - テストスコープ: TC-011 (`--all` clean baseline)、TC-012 (`--all` planted regression detection)、TC-013/TC-014/TC-015 (`--repo-root` flag の 3 経路 coverage: `/tmp` からの happy path で CWD 非依存性を検証、引数欠落で exit 2、存在しないディレクトリで exit 2) を追加し、`plugins/rite/scripts/tests/check-no-direct-gh-issue-create.test.sh` は 16 assertion 構成に。`references/issue-create-with-projects.md` の Static Guard 節は両モード (明示ファイルリスト / `--all`) と `--repo-root` override の両方を併記するよう更新。
...
- 横断的 orchestrator return-block implicit stop regression を mitigation (#910)
  - 用語注: 本エントリ中の数値表現は意味が異なるため以下のように呼び分ける — (a) **4 imperative-strengthened source file** (implicit stop 抑制のため imperative 強度を強化した 4 つの caller / sub-skill 出力点)、(b) **5 cross-orchestrator grep target (Issue #917 で 4 → 5 に拡張) + `create-interview.md` 内 3 supplementary pin type category (計 5 assertion: caller HTML literal positive pin 2 (TC-5.3 / TC-5.4) + anti-pattern revert 2 (TC-5.1 / TC-5.2) + plain-text reminder pin 1 (TC-5.5))** (test pin 箇所: 主 grep-target scope は `create.md` 内 2 箇所 + `cleanup.md` 1 箇所 + `ingest.md` 2 箇所、補完 pin scope は `create-interview.md` 単一ファイル内 5 assertion で asymmetric weakening 検出を担当 — 本 framing は `sub-skill-return-protocol.md` の "3 layer canonical signaling pattern" blockquote 末尾の **Granularity-mixing prohibition** note と整合: site 数 (5 cross-orchestrator) と assertion 数 (5 supplementary) は粒度が異なるため単純加算しない)、(c) **3 canonical Layer 3 sub-layer** (caller HTML hint = 3a / sub-skill plain-text reminder = 3b / sub-skill HTML continuation comment = 3c — `sub-skill-return-protocol.md` の "3 layer canonical signaling pattern" blockquote 参照、Defense-in-depth Layer 1/3 とは別概念)。
  ...
  - **4 imperative-strengthened source file (計 5 site — Issue #917 後は `ingest.md` が 2 site)** で imperative 強度を強化: `commands/issue/create-interview.md` caller HTML literal + plain-text continuation reminder (`継続中` → `MUST continue (turn を閉じない)` recast)、`commands/issue/create.md` Mandatory After Interview / Mandatory After Delegation prose、`commands/pr/cleanup.md` Mandatory After Wiki Ingest Step 0、`commands/wiki/ingest.md` continuation HTML comment + Mandatory After Auto-Lint Step 0 prose (後者は #917 で追加 — 後述「5 番目の canonical site に拡張」bullet 参照)。新 imperative keyword 群 (`MUST execute as VERY FIRST tool call BEFORE any text output, narrative, or response generation` / `DO NOT end the turn` / `DO NOT output any narrative text before this bash call`) で LLM の natural stopping point を消去する設計。
  ...
  - 横断 regression test `hooks/tests/step0-immediate-bash-presence.test.sh` 追加 (25 assertion: TC-1+2+3 で **14 main pin** — `create.md` ×4 + `cleanup.md` ×3 + `ingest.md` ×7 (Issue #917 で TC-3.5/3.6/3.7 を追加) — に加えて TC-4 で **3 cross-file count assertion**、TC-5 で **5 supplementary assertion** (`create-interview.md` caller HTML literal positive pin 2 (TC-5.3 / TC-5.4) + anti-pattern revert 2 (TC-5.1 / TC-5.2) + plain-text reminder pin 1 (TC-5.5) — 注: 5 件中 caller HTML literal を target するのは 3 件のみ (TC-5.2 / TC-5.3 / TC-5.4)、TC-5.1 と TC-5.5 は plain-text 内容を target) で asymmetric weakening 補完検出、**TC-6 で 3 件の lint.md Phase 9.2 imperative recast pin** (TC-6.1 line-start anchor 付き positive pin + TC-6.2 anti-pattern revert pin + TC-6.3 count pin (`>= 3` canonical site)、Issue #917 cycle 5/6 で `commands/wiki/lint.md` Layer 3b imperative recast の false-negative 経路を閉鎖するため追加。cycle 6 で TC-6.1 に line-start anchor を追加し rationale prose 内 inline backtick literal を排除、TC-6.3 で部分削除も検出)。presence + imperative keyword pin。byte equality は既存 `caller-html-literal-symmetry.test.sh` に委譲する責務分離維持)。
  ...
  - **5 番目の canonical site に拡張** (#917): PR #916 マージ直後の `/rite:pr:cleanup` 実機実行で、`rite:wiki:ingest` が `rite:wiki:lint --auto` return 後に implicit stop する事象を直接観測 (累積 27 回目)。Pre-#917 baseline では `commands/wiki/ingest.md` Mandatory After Auto-Lint Layer 1 prose を意図的に canonical phrasing 対象外として残置していた (older `MUST execute in the SAME response turn` 形式)。Issue #917 D-01 でその設計判断を反転し、本 site を 5 番目の canonical site に格上げ、`cleanup.md` Step 0/1 と byte-equal 相当の idempotent 二重 patch 構造 (`Step 0 Immediate Bash Action` 名称 + canonical `MUST execute as VERY FIRST tool call BEFORE any text output, narrative, or response generation` phrasing + `--preserve-error-count` + `--if-exists`) で対称化。`commands/wiki/lint.md` Phase 9.2 三点セット blockquote (3 site — Phase 1.1 早期 return / Phase 1.3 早期 return / Phase 9.2 documentation example) を現状報告 `> ⏭ 継続中:` → 命令形 `> ⏭ MUST continue (turn を閉じない):` に recast、Layer 3b plain-text reminder 強度を `commands/issue/create-interview.md` Return Output Format と対称化。`hooks/tests/step0-immediate-bash-presence.test.sh` を TC-3.5/3.6/3.7 (section anchor + 二重 patch 構造 + canonical Markdown bold pin) に拡張、TC-4.3 ingest.md count を `>= 1` → `>= 2` に bump。`skills/rite-workflow/references/sub-skill-return-protocol.md` Scope note + test scope description を 4 → 5 cross-orchestrator canonical site に更新し Issue #917 historical context blockquote を追加。Non-goal 遵守 (stop-guard.sh / multi-state API / sub-skill 三点セット構造 無変更)。
...
- `/rite:issue:create` ゼロベース再設計 (Phase E) を完了 (#823)
  ...
  - 詳細な完了レポートと AC 達成 evidence は `docs/designs/issue-create-zerobase-redesign.md` Section 11 を参照。
