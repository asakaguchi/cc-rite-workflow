...
# --------------------------------------------------------------------------
# T-LOCK-1 (verified-review C3): Advisory lock の static pin
#   `exec 9>` line does NOT contain `2>` redirect (cycle 11 で確立された
#   stderr 退避と fd 9 同時 redirect 禁止 doctrine の固定)
# --------------------------------------------------------------------------
echo "T-LOCK-1 (verified-review C3): advisory lock 'exec 9>' は 2> redirect を含まない"
if grep -E '^[[:space:]]*if exec 9>' "$flow_state_path" | grep -q '2>'; then
  fail "exec 9> 行に 2> redirect が混入している (cycle 11 で fix した silent failure が revert された可能性)"
else
  pass "exec 9> 行に 2> redirect なし (writer/reader 対称化 doctrine 維持)"
fi

# --------------------------------------------------------------------------
# T-LOCK-2 (verified-review C3): Advisory lock の static pin
#   command -v flock guard が存在し、busybox fallback path が WARNING emit する
# --------------------------------------------------------------------------
echo "T-LOCK-2 (verified-review C3): flock command guard + busybox fallback WARNING"
if grep -qE 'if command -v flock' "$flow_state_path" \
   && grep -qE 'WARNING: flock command 不在' "$flow_state_path"; then
  pass "flock command guard + busybox fallback WARNING が存在 (HIGH-1 race protection 退化検出)"
else
  fail "flock command guard / busybox fallback WARNING のいずれかが消失"
fi

# --------------------------------------------------------------------------
# T-LOCK-3 (verified-review C3): Advisory lock 30s timeout の static pin
# --------------------------------------------------------------------------
echo "T-LOCK-3 (verified-review C3): flock -w 30 timeout assertion"
if grep -qE 'flock -w 30 9' "$flow_state_path" \
   && grep -qE 'flow-state advisory lock 取得に失敗.*30s timeout' "$flow_state_path"; then
  pass "flock -w 30 + 30s timeout ERROR message が存在 (timeout 値 drift 検出)"
else
  fail "flock -w 30 の timeout assertion または ERROR message が消失"
fi

# --------------------------------------------------------------------------
# T-LOCK-4 (verified-review C3 + I-1): Advisory lock 取得は --if-exists validation の後
#   旧実装は flock 取得が validation 前にあり、--if-exists skip path で
#   `.flow-state.lock` が disk に残留していた (code-reviewer I-1)。
#   静的 pin: 'flow_state_advisory_lock' anchor が 'INFO: --if-exists' lines よりも
#   後に出現することを確認 (構造的に validation block の後に lock 取得があることを保証)。
# --------------------------------------------------------------------------
echo "T-LOCK-4 (verified-review C3+I-1): advisory lock acquisition is AFTER --if-exists validation"
_lock_anchor_line=$(grep -n 'DRIFT-CHECK ANCHOR: flow_state_advisory_lock' "$flow_state_path" | head -1 | cut -d: -f1)
_if_exists_first_line=$(grep -n 'INFO: --if-exists patch skipped' "$flow_state_path" | head -1 | cut -d: -f1)
if [ -n "$_lock_anchor_line" ] && [ -n "$_if_exists_first_line" ] && \
   [ "$_lock_anchor_line" -gt "$_if_exists_first_line" ]; then
  pass "advisory_lock anchor (L$_lock_anchor_line) is after --if-exists patch skip emit (L$_if_exists_first_line) — I-1 lock leak fix 維持"
else
  fail "advisory_lock anchor が --if-exists validation より前にある (lock leak 修正が revert された可能性)"
fi
unset _lock_anchor_line _if_exists_first_line

# --------------------------------------------------------------------------
# T-LOCK-5 (verified-review C3 + I-1): --if-exists skip path で lock file が残らない
#   Runtime test: file 不在状態で --if-exists patch を実行し、
#   親ディレクトリに `.flow-state.lock` が作成されないことを確認。
# --------------------------------------------------------------------------
echo "T-LOCK-5 (verified-review C3+I-1): --if-exists patch skip leaves no lock file"
TD=$(make_test_dir); cleanup_dirs+=("$TD")
write_config "$TD" 2
SID="55555555-5555-5555-5555-555555555555"
write_session_id "$TD" "$SID"
# state file を作成しない状態で --if-exists patch を実行
# lock file が作成されないことを確認
(cd "$TD" && bash "$HOOK" patch \
  --phase "phase_test" --next "next_test" --if-exists >/dev/null 2>&1) || true
_lock_count=$(find "$TD" -name "*.flow-state.lock" 2>/dev/null | wc -l)
if [ "$_lock_count" -eq 0 ]; then
  pass "--if-exists patch skip 後に .flow-state.lock が残留しない (I-1 lock leak 防止)"
else
  fail "--if-exists patch skip 後に .flow-state.lock が ${_lock_count} 件残留 (I-1 lock leak が再発)"
fi
unset _lock_count
...
SID="66666666-6666-6666-6666-666666666666"
write_session_id "$TD" "$SID"
# まず create で正常な state file を作成
(cd "$TD" && bash "$HOOK" create \
  --phase "create_interview" --issue 100 --branch "test" \
  --pr 0 --next "test" >/dev/null 2>&1) || true
# state file を 0-byte に truncate
STATE_FILE_PATH="$TD/.rite/sessions/$SID.flow-state"
if [ -f "$STATE_FILE_PATH" ]; then
  : > "$STATE_FILE_PATH"
  _empty_stderr=$(mktemp)
  rc=0
  (cd "$TD" && bash "$HOOK" patch \
    --phase "create_interview" --next "test" 2>"$_empty_stderr") || rc=$?
  if [ "$rc" -eq 1 ] && grep -q "ERROR.*flow-state file exists but is empty" "$_empty_stderr"; then
    pass "patch mode: 0-byte state file → rc=1 + ERROR message (C5 empty-file guard 維持)"
  else
    fail "patch mode 0-byte: rc=$rc, stderr=$(head -3 "$_empty_stderr")"
  fi
  rm -f "$_empty_stderr"
else
  fail "state file が create 後に存在しない (T-EMPTY-1 prerequisite 失敗)"
fi

# --------------------------------------------------------------------------
# T-EMPTY-2 (verified-review C5): increment mode で 0-byte state file は ERROR exit 1
# --------------------------------------------------------------------------
echo "T-EMPTY-2 (verified-review C5): increment mode + 0-byte state file → rc=1 + ERROR text"
TD=$(make_test_dir); cleanup_dirs+=("$TD")
write_config "$TD" 2
SID="77777777-7777-7777-7777-777777777777"
write_session_id "$TD" "$SID"
(cd "$TD" && bash "$HOOK" create \
  --phase "phase_init" --issue 100 --branch "test" \
  --pr 0 --next "test" >/dev/null 2>&1) || true
STATE_FILE_PATH="$TD/.rite/sessions/$SID.flow-state"
if [ -f "$STATE_FILE_PATH" ]; then
  : > "$STATE_FILE_PATH"
  _empty_stderr=$(mktemp)
  rc=0
  (cd "$TD" && bash "$HOOK" increment --field implementation_round 2>"$_empty_stderr") || rc=$?
  if [ "$rc" -eq 1 ] && grep -q "ERROR.*flow-state file exists but is empty" "$_empty_stderr"; then
    pass "increment mode: 0-byte state file → rc=1 + ERROR message (C5 empty-file guard 維持)"
  else
    fail "increment mode 0-byte: rc=$rc, stderr=$(head -3 "$_empty_stderr")"
  fi
  rm -f "$_empty_stderr"
else
  fail "state file が create 後に存在しない (T-EMPTY-2 prerequisite 失敗)"
fi

# --------------------------------------------------------------------------
# T-INFO-1 (verified-review C6 降格): --if-exists skip の INFO emit
# --------------------------------------------------------------------------
echo "T-INFO-1 (verified-review C6): --if-exists patch skip emits INFO to stderr"
TD=$(make_test_dir); cleanup_dirs+=("$TD")
write_config "$TD" 2
SID="88888888-8888-8888-8888-888888888888"
write_session_id "$TD" "$SID"
...
rc=0
(cd "$TD" && bash "$HOOK" patch \
  --phase "test" --next "test" --if-exists 2>"$_info_stderr") || rc=$?
if [ "$rc" -eq 0 ] && grep -q "INFO: --if-exists patch skipped" "$_info_stderr"; then
  pass "patch --if-exists skip: rc=0 + INFO line emitted (audit-trail 維持)"
else
  fail "patch --if-exists skip: rc=$rc, stderr=$(head -3 "$_info_stderr")"
fi
rm -f "$_info_stderr"
# increment mode も対称的に確認
_info_stderr2=$(mktemp)
rc=0
(cd "$TD" && bash "$HOOK" increment --field implementation_round --if-exists 2>"$_info_stderr2") || rc=$?
if [ "$rc" -eq 0 ] && grep -q "INFO: --if-exists increment skipped" "$_info_stderr2"; then
  pass "increment --if-exists skip: rc=0 + INFO line emitted (audit-trail 対称化)"
else
  fail "increment --if-exists skip: rc=$rc, stderr=$(head -3 "$_info_stderr2")"
fi
rm -f "$_info_stderr2"
