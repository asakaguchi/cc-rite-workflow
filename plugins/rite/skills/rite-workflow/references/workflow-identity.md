...
実 caller 配備の現状: 旧 `cleanup.md Phase 4.W.2` の blockquote style は defense 層物理排除で削除済。現在の caller は 2 style で配備されている (`grep -rn 'workflow-identity' plugins/rite/commands/` で確認可):
