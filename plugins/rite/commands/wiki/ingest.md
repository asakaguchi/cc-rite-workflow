...
> **Rationale**: caller (cleanup/review/fix/close) が implicit stop し `continue` 介入が必要となる症状の根本原因は、LLM が sub-skill return tag (`<!-- [lint:completed:auto] -->`) を turn 境界として誤認する turn-boundary heuristic の発火。Step 0 は **具体的な bash tool 呼び出し** を sub-skill return 直後の必須アクションとして挿入することで turn 境界シグナルを消去する。Step 0 / Step 1 は idempotent — この冗長性が防御機構である (cleanup.md Step 0/1 と同型、片側更新時は対称先も同時更新が必要、現状は test で pin されない interim 期間)。**Note (ADR docs/designs/parent-routing-unification.md)**: 旧 `hooks/tests/step0-immediate-bash-presence.test.sh` は parent-routing pattern 移行に伴い撤去済。本 site (`wiki/ingest.md` Mandatory After Auto-Lint Step 0/1) は PR-3 で parent-routing pattern に移行予定 (ADR Implementation Strategy table)、PR-7 で `parent-routing-pattern-uniformity.test.sh` が代替の対称性 pin となる。
