...
本 ADR は PR-1 として ADR-only で提出し、実装は PR-2 以降 (8 PR 構成) で段階的に進める。坂口さんの判断により Phase 分割 / PR-2 分割は採用せず、**8 PR 線形マージで一括実施**。実証失敗時の fallback として Plan B (stop-guard 軽量復活) を記憶。
...
rite plugin 内には 20 経路の sub-skill 呼び出しがあるが、implicit stop が累積系列として報告されているのは **3 経路だけ**:
...
| Layer | 撤去対象 |
|---|---|
| Layer 1 (orchestrator prompt contract) | `create.md` Mandatory After Interview / Mandatory After Delegation、`pr/cleanup.md` Mandatory After Wiki Ingest、`wiki/ingest.md` Mandatory After Auto-Lint |
...
| Invariant test | `4-site-symmetry.test.sh` / `caller-html-literal-symmetry.test.sh` / `step0-immediate-bash-presence.test.sh` / `create-interview-responsibility-separation.test.sh` / `caller-html-literal-symmetry-decompose-register.test.sh` (合計 -843 LoC) |
