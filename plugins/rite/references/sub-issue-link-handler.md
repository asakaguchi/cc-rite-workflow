...
GitHub Sub-issues API で親 Issue と子 Issue を紐付けた結果 (`link_status`) をハンドルするロジック。`/rite:issue:create` が flat 化される前は `create-decompose.md` と `parent-routing.md` の 2 ヶ所に inline 重複していたが、現在は `commands/issue/create.md` ステップ 5.4 (Decompose Path bulk create loop) のみが使用する。
...
単発の子 Issue を紐付け、失敗集計を行わないケース。`parent-routing.md` sub-skill (`/rite:issue:start` 内部) の child creation path で使われていた (現在は flat 化で sub-skill 自体が削除)。現状は主に Variant B (counting) の方が `commands/issue/create.md` ステップ 5.4 で使われる。
...
複数の子 Issue を loop で紐付け、全件失敗 / 部分失敗を別レイヤで検出するケース。`commands/issue/create.md` ステップ 5.4 (Decompose Path bulk create loop、`create-decompose.md` Phase 3.3 から移管 (現在は flat workflow に統合済)) で使用する。呼び出し元は loop の前に `link_failures=0` で初期化しておくこと。
...
- `commands/issue/create.md` ステップ 5.4 — Decompose Path bulk create loop (Variant B 利用、flat workflow 統合後の単一 caller)
...
- [`commands/issue/create.md`](../commands/issue/create.md) ステップ 5.4 (Sub-Issues API Linkage in Decompose Path) — Variant B の利用箇所 (旧 `create-decompose.md` Phase 3.3 を flat workflow に統合)
- 旧 caller (retired): `parent-routing.md` child creation path (Variant A) / `create-decompose.md` Phase 3.3 (Variant B)
