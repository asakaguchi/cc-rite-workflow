...
- 旧 sub-skill chain アーキテクチャに紐付いた orphan hook test 18 件を #1079 で削除 (削除済み hook 名を直接持つ 4 件 + アーキテクチャ依存の 13 件 + 削除済み `start-execute.md` / `start-publish.md` / `start-finalize.md` を参照していた `start-md-charter.test.sh`。後者は `count_aggregated` が不在ファイルを silent skip するため見かけ上 PASS していたが、aggregated 下限 assert が 0 計上で機能停止していた)。同テストのみが消費していた `tests/fixtures/start-md/` も削除。flat workflow 向けの新規テストは本 PR 内で追加済み (`incident-emit-callsite-coverage`、`start-md-sentinel-coverage`、`phase-transition-whitelist`、`issue-body-safe-update`、`link-sub-issue`、`post-compact-classification-regex`)。
