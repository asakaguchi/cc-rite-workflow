...
| 英語 | `In the old code ...` 等、過去の実装と比較する記述 (本原則 Failure Patterns の用例の英語版) |
| 日本語 | 「コミット {sha} で対応しました」「PR #{N} で対応」「#{N} で別途対応」 |
| 日本語 | 「サイクル N で導入」「cycle N F-M で確立」等 |
| 日本語 | 「旧実装は ...」「旧コードでは ...」等、過去の実装と比較する記述 (本原則 Failure Patterns の用例 `# 旧実装は ～。PR #N で修正` と対応) |
