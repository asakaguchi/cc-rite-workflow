...
### 5.6.3 Recommendation Disposition Reporting (Issue #1042)

> **Purpose**: aggregate label (「推奨 N 件」「follow-up 候補 N 件」「全て scope 外」) で推奨事項を済ませる responsibility-obscuring path を遮断し、各 item の disposition (actionable → Issue 番号 / boundary → 件数 / design_confirmation → 件数) を completion report に明示する。これは Issue #1042 の AC-3 (disposition breakdown 必須化、aggregate「推奨 N 件」行廃止、責任曖昧化 phrase 禁止) を満たす機械的契約。

> **Output ordering** (must match `completion-report.md` Step 3.4): This section is appended **after the case-specific sections (項目テーブル + フェーズ進捗) but before 次のステップ section**, i.e., directly after フェーズ進捗 table. The runtime sequence is: 項目テーブル → フェーズ進捗 → **5.6.3 推奨事項 disposition** → 次のステップ → 5.6.2 (Wiki ingest 状況) → 5.6.1 (workflow incidents).

**Source of truth**: `recommendation_items` collected by `pr/review.md` Phase 5.1 (latest cycle) — the conversation context retains this list with `reviewer_type / classification / content / file_line` per item. Additionally, `new_issue_number` values from Phase 7.4 (Issue creation) are aggregated for actionable items that were ✅ approved by user in Phase 7.2.

**Step 1 — Aggregate by classification**:

Iterate `recommendation_items` (from latest review cycle) and group by `classification`:

| Classification | Count | Detail |
|----------------|-------|--------|
| `actionable` | `actionable_count` | List of `new_issue_number` from Phase 7.4 (e.g., `#1040, #1041`) |
| `boundary` | `boundary_count` | Per-item disposition from Phase 7.2 AskUserQuestion responses (e.g., `2 件: 1 件 Issue 化済 #N / 1 件無視`) |
| `design_confirmation` | `design_confirmation_count` | Summary of "no action" rationale (e.g., `3 件: reviewer 自身が対応不要と結論`) |

**Step 2 — Output**:

Append the disposition table to the completion report between the フェーズ進捗 table and the 次のステップ section (per `completion-report.md` Step 3.4). When **all three counts are 0**, output `_推奨事項なし_` instead of the table (minimize output). When **any count >= 1**, the full table MUST be rendered with all three rows (even if some rows have count 0 — explicit zero is itself a signal worth showing).

**Step 3 — Prohibition enforcement (MANDATORY)**:

The following phrases are **PROHIBITED** in the completion report:
  ...
  if [ "{pr_number}" != "0" ] && [ -n "{pr_number}" ]; then
    ...
    if [ "$PROJECTS_ENABLED" = "true" ] && [ -n "$PROJECT_NUMBER" ]; then
      ...
      if PR_IS_DRAFT=$(gh pr view {pr_number} --json isDraft --jq '.isDraft // null' 2>"${pr_view_err:-/dev/null}"); then
        ...
        # Issue #1008 cycle 11 F-01 4-site 対称化: post-compact.sh / start.md Step 1.5 と同様に、PR が close/merge/delete されている
        ...
        if printf '%s' "$pr_err_oneline" | grep -qiE 'could not resolve.*pull\s*request|no.*pull\s*request found'; then
          ...
          pr_root_cause_hint="gh_api_failure_at_final_fallback"
        fi
        bash {plugin_root}/hooks/workflow-incident-emit.sh \
          --type projects_status_in_review_missing \
