...
> **flow-state ring**: 本 Phase 8.2 は lint Skill invoke 直前に flow state の `.phase = ingest_pre_lint` (active=true) に patch します。caller 経由時は caller phase (例: `cleanup_pre_ingest`) を一時上書きし、sub-skill return 後 Mandatory After Auto-Lint Step 1 が `ingest_post_lint` に patch → caller Mandatory After が caller phase (`cleanup_post_ingest`) に書き戻す ring 構造です。単独実行時は flow state 不在のため `--if-exists` で no-op (従来互換)。phase tagging により、protocol violation が起きた場合に retrospective workflow_incident detection が `ingest_*` 由来の incident として correlate できる。

**Pre-write** (before invoking `rite:wiki:lint --auto`): Update flow state to `ingest_pre_lint` so the sub-skill flow is tagged with the ingest phase. The `if ! cmd; then` rc capture is mandatory — silent patch failure leaves the sub-skill flow untagged, so the retrospective `manual_fallback_adopted` correlation in `start.md` ステップ 8.5 has no phase to attribute the incident to:
...
if ! bash {plugin_root}/hooks/flow-state.sh set \
 ...
 --if-exists; then
 echo "WARNING: flow-state.sh set (ingest_pre_lint) failed — sub-skill will run unphased, so retrospective workflow-incident correlation against ingest_pre_lint will not fire. Investigate the helper exit reason in stderr above before relying on this protection again." >&2
fi
...
<!-- >>> DRIFT-CHECK ANCHOR: 🚨 Mandatory After Auto-Lint ring <<<
 ...
 いずれかが成功することを保証する。
 phase-transition-whitelist.sh の `ingest_pre_lint` / `ingest_post_lint` entry は本 ring の
 phase tagging を支える。いずれかを変更する場合、以下 2 site を同時確認:
...
**Step 1**: Update flow state to `ingest_post_lint` phase (idempotent re-patch)。Step 0 が既に書いた `ingest_post_lint` の timestamp / `next_action` を refresh する。2 重 patch design は transient failure 下でも Step 0 / Step 1 のいずれかが成功することを保証し、同時失敗時のみ `[CONTEXT] STEP_1_PATCH_FAILED=1` を retained flag として残す。`--preserve-error-count` は Step 0 と対称に付与 (`.error_count` は現状 half-legacy schema slot で production reader 不在だが、future reader 再導入時の累積カウントを失わないよう保持する):
...
> **設計判断 — flow-state ring** (Issue #618 / #917, supersedes the former YAGNI stance): `ingest.md` は Phase 8.2 Pre-write (`ingest_pre_lint`)、🚨 Mandatory After Auto-Lint Step 0 + Step 1 (idempotent 二重 patch、`ingest_post_lint` — Step 0 が FIRST patch、Step 1 が idempotent retry、Issue #917 で 5 site canonical 対称化)、Phase 9.1 Step 3 (`ingest_completed`, active=false) の 3 logical patch site (4 physical bash call) で flow-state.sh を呼ぶ。caller 経由時は caller phase を一時上書きし、sub-skill return 後 caller Mandatory After (例: cleanup.md `🚨 Mandatory After Wiki Ingest`) が caller phase (`cleanup_post_ingest`) に書き戻す ring 構造で完遂する。単独実行時は `--if-exists` により flow-state 不在なら no-op (従来互換)。本設計反転は Issue #618 (ingest→lint return 後の implicit stop を多層防御で防ぐ) の AC-2/AC-3 要件に対応。
...
if ! bash {plugin_root}/hooks/flow-state.sh set \
 ...
 --if-exists; then
 echo "WARNING: flow-state.sh set (ingest_completed) failed — flow-state may still report ingest_post_lint. Caller Mandatory After will still be able to write back its own phase via --if-exists. Investigate the helper exit reason in stderr above." >&2
fi
