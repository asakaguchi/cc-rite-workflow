...
> **構造的保証 (Issue #1176)**: 終了 sentinel (`[review:mergeable]` / `[fix:replied-only]` / `[fix:cancelled-by-user]`) 到達時、sub-skill が `FINALIZE:...` handoff をセットしており、`Stop` hook が本ステップの完了通知を出力せず turn を終えようとする停止を **1 回だけ** 差し戻す。詳細は「ループ継続・終了の構造的保証」節を参照。完了通知は必ず出力すること。
...
## ループ継続・終了の構造的保証 (Issue #1168 / #1176)

ステップ 2 / ステップ 4 の「次のコマンドへ進む」遷移 (継続点) と ステップ 5 の完了通知 (終了点) は本来 LLM が prose 指示 ("Do NOT stop") に従って自走するが、LLM が sentinel を出した直後に turn を終了する中断が観測された (継続点: PR #1167 / 終了点: PR #1175 で `[review:mergeable]` 到達後に完了通知を出さず停止)。これを防ぐため、prose に依存しない **構造的な層** を `Stop` hook で実装している。継続点と終了点で **対称** に handoff をセットする:
...
- **終了 handoff (FINALIZE, one-shot — Issue #1176)**: 終了 sentinel を出す sub-skill が flow-state に `FINALIZE:{result}:{pr}` handoff をセットする。
...
- **無限 block ループ防止**: handoff は consume で one-shot 消費される。進捗 (次コマンド実行 / 完了通知出力) の後に再度停止すれば handoff は空 → block しない。cycle counter は持たない (#1136 と整合 — 安全網は終了 sentinel + Ctrl+C に委ねる)。終了 handoff も同じ one-shot 契約で **1 回だけ** block するため、完了通知を強制しても無限 block にはならない。
