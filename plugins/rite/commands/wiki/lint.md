...
> **Reference**: canonical 実装は `plugins/rite/hooks/scripts/wiki-lint-stale.sh`。helper は GNU date 検査・branch_strategy 別のページ読出 (`git show`(separate_branch) / `cat`(same_branch))・`updated` frontmatter 抽出・date パース失敗の WARNING skip・marker block / `stale_check_ok` enum / `[CONTEXT]` sentinel 出力をすべて内包する (旧ステップ 1.2 + 4.2 inline 実装を委譲)。placeholder residue gate も helper 内で実行される。state machine 契約 (marker block + enum) は `references/bash-cross-boundary-state-transfer.md` の Pattern 1/2 を参照。
