...
> **Charter**: Subject to [Simplification Charter](../../../skills/rite-workflow/references/simplification-charter.md). Runtime に効かない経緯記述・cycle 番号引用・重複 confirmation は書かない。
...
> **⚠️ コード層との境界**: 本 reference は anchor の **literal 構造** (文字列としての形態) を規定する。anchor が指し示す bash block の動作仕様・契約は anchor を抱える各文書 (`pr/fix.md` / `pr/review.md` / `issue/close.md` / `issue/references/bulk-create-pattern.md`) に存在する。anchor literal を変更する場合は本 reference 更新後に、同 anchor を citation する全 site (note / blockquote / 他 anchor の rationale 段落) を grep で検出し同時更新すること。
...
(出典: `commands/pr/fix.md:1083-1085`)
...
(出典: `commands/pr/fix.md:1118` — refactor 候補として §4 audit に列挙)
...
| `commands/issue/references/bulk-create-pattern.md` | 43 | `# === Loop pre-amble (このブロックは結合スクリプトの先頭で1回だけ実行) ===` |
| `commands/issue/references/bulk-create-pattern.md` | 55 | `# === Per-Sub-Issue body (N 回複製して連結。各複製ごとに placeholder を実値で置換) ===` |
...
- `commands/issue/references/bulk-create-pattern.md:43,55` → anchor を `# === Loop pre-amble ===` / `# === Per-Sub-Issue body ===` に最小化し、用途説明は直前コメント行へ移動
...
| `commands/issue/references/bulk-create-pattern.md` | 113 | `# === Per-Sub-Issue body 終了。次の Sub-Issue があれば、ここに次の複製を連結する ===` (※末尾は説明的だがブロック終端マーカーとして許容) |
...
| SoT 文書の path 参照は本 PR マージ時点の origin/develop で existence check する | `sot-path-reference-existence-check`, `broken-ref` | 本 reference 内の path 参照 (`commands/pr/fix.md:1118` 等) は本 PR マージ時点の origin/develop で実在を verify した上で記載 |
