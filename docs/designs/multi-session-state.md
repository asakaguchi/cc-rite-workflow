...
- `handoff` (Issue #1168 で導入。review↔fix ループの one-shot 継続マーカー。`flow-state.sh set --handoff <cmd>` が writer で **毎 set デフォルトクリア** (= `--handoff` 指定時のみ key を付与、`error_count` と同じ phase-transition クリア semantics)。`flow-state.sh consume-handoff` が reader+deleter で、`Stop` hook `stop-loop-continuation.sh` が turn 終了時に呼ぶ。継続 sentinel を出す `review.md` Step 8.0 / `fix.md` Step 5.1 のみがセットし、終了 sentinel はセットしないため誤継続しない。additive かつ後方互換 (`.handoff // ""`) のため `schema_version` bump 不要)
