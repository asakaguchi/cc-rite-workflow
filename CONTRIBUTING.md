...
1. **`plugins/rite/agents/{type}-reviewer.md`** (new file) — the reviewer's full profile (Role / Core Principles / Detection Process / Detailed Checklist / Output Format), injected as the named subagent's system prompt. Model an existing reviewer (e.g. `security-reviewer.md`); shared principles live in `_reviewer-base.md` and must not be duplicated.
