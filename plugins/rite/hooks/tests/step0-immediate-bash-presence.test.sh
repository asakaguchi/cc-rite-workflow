...
# F-05 (cycle 4): INTERVIEW_MD を precondition guard に組み込み、silent skip による regression 見逃しを防ぐ。
...
# F-08 (cycle 1): 当初コメントは「両方を許容」と書かれていたが、ERE は大文字小文字を区別するため
...
# 兆候として fail させる。
#
# F-21 (cycle 4 LOW): Mandatory After Delegation prose (create.md:304) の `**VERY FIRST tool
# call (cognitive action)**` variant も canonical pin に含めるため、alternation regex で
# 2 種の bold パターン両方を許容する形式に拡張。
...
# F-04 (cycle 1), F-10 (cycle 3 LOW): Step 0 specific anchor + alternation を 2 つの assert_grep に分割して
# specificity を上げる。
#
# 設計意図 (cycle 4 で正確化):
...
# F-02 (cycle 3 HIGH): TC-3.2/3.3/3.4 は **`<!-- continuation:` で始まる単一行内** に keyword が
...
# F-03 (cycle 3 MEDIUM): TC-4.1 は当初 file 横断合計 `>=4` という緩い閾値だったため、
...
# F-05 (cycle 4): INTERVIEW_MD は precondition guard ループ (本ファイル冒頭の file 存在 hard error)
...
# F-07 (cycle 2): TC-5.2 — caller HTML literal 内の旧 phrasing revert 検出。
...
# F-02 (cycle 3 HIGH): TC-5.3 / TC-5.4 — caller HTML literal 内の **positive imperative keyword** pin。
...
# F-18 (cycle 4 LOW): Layer 3b (sub-skill plain-text reminder) imperative 強度 pin。
# create-interview.md の `> ⏭ MUST continue (turn を閉じない):` plain-text reminder は
# CHANGELOG で「load-bearing 設計の Layer 3b」と明記されているが、TC-5.1 (anti-pattern) のみで
# 守られていた。reverter が `継続中` (without `自動継続します` suffix) や別 weak-phrasing に
# reset しても TC-5.1 は pass してしまう false-negative を防ぐため、positive presence pin を追加。
assert_grep "TC-5.5: create-interview.md plain-text reminder に 'MUST continue' が存在 (Layer 3b imperative 強度 pin)" \
  "$INTERVIEW_MD" \
  '⏭ MUST continue'
