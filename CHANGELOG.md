...
- **Flipped `multi_session.enabled` default from `false` to `true` — multi-session worktree isolation is now on by default** (#1391) — a deliberate **design-philosophy shift**: Worktree Mode (Epic #1360, hardened through #1381–#1388) was built and documented as "opt-in / default off / zero behavior change when off"; with the feature now production-ready, the `/rite:init` config template ships `multi_session.enabled: true` so new projects get per-session Git worktree isolation (one terminal = one Issue, `.rite/worktrees/issue-{N}`) automatically. This structurally prevents working-tree cross-contamination and branch mix-ups across concurrent sessions. **Backward compatibility is preserved**: `pr:open`'s config parser still falls back to `false` when the `multi_session:` block is entirely absent, so existing projects whose configs predate the feature are unaffected — the new default only takes effect for freshly generated `/rite:init` configs. Setting `enabled: false` explicitly restores the pre-#1391 single-session behavior (byte-identical). The EnterWorktree opt-in guard ("explicit user or project instruction only") remains satisfied under default-on because `multi_session.enabled: true` in a committed `rite-config.yml` is itself a project instruction, regardless of whether the value came from the template default or an explicit edit. Disk cost note (each session worktree is a full working-tree clone; `node_modules`/`venv`/build caches are not shared) is retained in the template comment and the on-demand FAQ. Affected files: `rite-config.yml` (dogfood repo), `plugins/rite/templates/config/rite-config.yml` (block moved above the `--- Advanced ---` marker so `/rite:init` new-generation emits it), `commands/pr/open.md` (steps 1.4 / 2.2 / 2.3-W), `commands/getting-started.md` (Phase 4.5 FAQ), `commands/workflow.md` (Tips block), `commands/init.md` (Step 4 `--upgrade` classification — `multi_session` excluded from back-add to preserve backward compat), `references/git-worktree-patterns.md`, `docs/CONFIGURATION.md`, `docs/SPEC.md`, `docs/designs/multi-session-worktree.md`. `.gitignore` requirement (`.rite/worktrees/`) is already emitted by `/rite:init` and verified non-warning by `gitignore-health-check.sh` under default-on.
- **Renamed all skill return sentinels from `:completed` to `:returned-to-caller`** (#1165) — the literal `completed` in sub-skill output patterns (`[lint:completed:auto]`, `[ingest:completed]`, `[cleanup:completed]`, `[create:completed:{N}]`, `[ready:completed]`, `[merge:completed]`) was identified as colliding with the LLM's turn-boundary heuristic — outputting `completed` as the final line caused the orchestrator to incorrectly close the turn, dropping `cleanup → wiki:ingest → wiki:lint` and similar nested flows mid-execution (PR #1164 cleanup, multiple recurrences across #604 / #618 / #923 / #1144 / #1163). Replaced the terminal vocabulary with `returned-to-caller`, which encodes the nested-call semantic ("returned to caller = caller must continue next step") directly into the literal. Each emit site now additionally prefixes `<!-- skill return signal: caller must continue next step -->` as an active disambiguation marker. In addition, all sentinel literals themselves are wrapped in HTML comments (`<!-- [skill:returned-to-caller] -->`) so that the user-visible terminus is never a bare bracket sentinel — both the disambiguator and the sentinel are now HTML-commented across every emit site (`create.md` / `cleanup.md` / `ingest.md` / `ready.md` / `merge.md` / `wiki/lint.md`). The ingest-side parser (`^Lint: contradictions=` regex in `wiki/ingest.md` ステップ 8.3) only depends on the first data line and was not affected by the sentinel rename — verified before rollout. Two meta-tests were added to mechanically prevent recurrence of related drift patterns: (1) `hooks/tests/wiki-lint-ingest-contract-sync.test.sh` asserts `lint.md` ↔ `ingest.md` line-count contract synchronisation (Asymmetric Fix Transcription anti-pattern; includes TC-1b which `grep -c = 1`-checks SoT single-site invariant for `lint.md` ステップ 9.2), and (2) `hooks/tests/sentinel-disambiguator-adjacency.test.sh` asserts disambiguator/sentinel count parity across the 5 non-`create.md` producers (`cleanup` / `merge` / `ready` / `wiki/lint` / `wiki/ingest`) — complementing the existing `create-md-invocation-symmetry.test.sh` TC-7a/7b adjacency check so silent marker strip is mechanically detected for every producer. Affected files: `commands/wiki/{ingest,lint}.md`, `commands/pr/{cleanup,ready,merge}.md`, `commands/issue/{create.md,references/bulk-create-pattern.md}`, `commands/sprint/execute.md`, `skills/rite-workflow/SKILL.md` + `references/{workflow-identity,sub-skill-return-protocol}.md`, `hooks/tests/{create-md-invocation-symmetry,wiki-lint-ingest-contract-sync,sentinel-disambiguator-adjacency}.test.sh`, `hooks/scripts/tests/test-hardcoded-line-number-check.sh`, plus matching updates to `docs/SPEC.md`, `docs/anti-patterns/cleanup-wiki-ingest-turn-boundary.md`, and `docs/tests/rite-issue-create-e2e-smoke.test.md`. Backward compat: historical raw sources / log.md / wiki history retain the old literals as-is per `historical preservation` policy. The residual-old-sentinel grep check lives in the manual smoke procedure (`docs/tests/rite-issue-create-e2e-smoke.test.md` scenario 3); the `hooks/tests/` meta-tests assert the new `returned-to-caller` sentinel presence and disambiguator parity (they do not auto-detect residual old literals). No runtime hook blocks flows. In-flight flows still on the old sentinel path must restart via `/rite:resume`, because the new command / SKILL definitions no longer recognize the old literals as continuation triggers (the orchestrator's sentinel matching no longer matches).
...
- New sentinels: `[merge:returned-to-caller]` / `[merge:not-ready]` / `[merge:error]` (specific to `/rite:pr:merge`; renamed from `[merge:completed]` in #1165)
...
- 17 orphan hook tests removed in #1079 (3 tests directly named after deleted hooks + 13 tests structurally tied to the deleted sub-skill chain + `start-md-charter.test.sh` which referenced deleted `start-execute.md` / `start-publish.md` / `start-finalize.md` and silently passed only because `count_aggregated` skipped non-existent files). Also removed `tests/fixtures/start-md/` (only consumed by the charter test). New tests for the flat workflow are added inline (`incident-emit-callsite-coverage`, `start-md-sentinel-coverage`, `phase-transition-whitelist`, `issue-body-safe-update`, `link-sub-issue`, `post-compact-classification-regex`, `create-md-invocation-symmetry`, `resume-md-phase-mapping`). Note: `step0-imperative-phrasing-cleanup-ingest` was added in #1079 but retired in #1149 (Issue #1144) along with the defense layers it pinned — the test and the pinned literals were removed together as part of the structural caller-chain solution.
...
- New Stop event hook `hooks/stop-create-interview-block.sh` as a machine-enforced back-stop for `/rite:issue:create` implicit stop (#920, cumulative series #910 / #917 / #634 / #651 / #622 / #552 / #687) — **removed in #1079 before release**
  ...
  - Pinned by new `hooks/tests/stop-create-interview-block.test.sh` (9 TC, 24 assertions): gate match → exit 2 + ACTION + sentinel; phase mismatch / active=false / pr_number≠0 / stop_hook_active / state-file absent / non-Stop event → exit 0 allow; `workflow_incident.enabled=false` → block respected, sentinel suppressed; `rite-config.yml` exists but `workflow_incident:` section absent → block + sentinel emit (default-on respected; regression guard for strict-mode + ERR-trap compatibility).
...
- Mitigated cross-orchestrator return-block implicit stop regression (#910)
  - Terminology note: this entry uses three counts that look similar but mean different things — (a) **4 imperative-strengthened source files** (the caller / sub-skill output points modified to suppress implicit stop), (b) **5 cross-orchestrator grep targets (extended from 4 to 5 in Issue #917) + 3 supplementary pin type categories in `create-interview.md` (5 assertions total: 2 caller HTML literal positive pins (TC-5.3 / TC-5.4) + 2 anti-pattern revert checks (TC-5.1 / TC-5.2) + 1 plain-text reminder pin (TC-5.5))** (test pin positions: 2 in `create.md` + 1 in `cleanup.md` + 2 in `ingest.md` as the main grep-target scope; supplementary pin scope: all 5 assertions reside intra-file in `create-interview.md` for asymmetric-weakening detection — this framing is aligned with the **Granularity-mixing prohibition** note at the end of `sub-skill-return-protocol.md`'s "3 layer canonical signaling pattern" blockquote: site count (5 cross-orchestrator) and assertion count (5 supplementary) are different granularities and must not be summed), (c) **3 canonical Layer 3 sub-layers** (caller HTML hint = 3a / sub-skill plain-text reminder = 3b / sub-skill HTML continuation comment = 3c — see `sub-skill-return-protocol.md` "3 layer canonical signaling pattern" blockquote for details, distinct from Defense-in-depth Layer 1/3 numbering).
  ...
  - Added cross-orchestrator regression test `hooks/tests/step0-immediate-bash-presence.test.sh` (25 assertions: TC-1+2+3 for **14 main pins** — `create.md` ×4 + `cleanup.md` ×3 + `ingest.md` ×7 (TC-3.5/3.6/3.7 added by Issue #917) — plus TC-4 for **3 cross-file count assertions**, TC-5 for **5 supplementary assertions** over `create-interview.md` (2 caller HTML literal positive pins (TC-5.3 / TC-5.4) + 2 anti-pattern revert checks (TC-5.1 / TC-5.2) + 1 plain-text reminder pin (TC-5.5) — note: only 3/5 target the caller HTML literal (TC-5.2 / TC-5.3 / TC-5.4); TC-5.1 / TC-5.5 target plain-text content) for complementary asymmetric-weakening detection, and **TC-6 for 3 lint.md Phase 9.2 imperative recast pins** (TC-6.1 line-start-anchored positive pin + TC-6.2 anti-pattern revert pin + TC-6.3 count pin (`>= 3` canonical sites) — added by Issue #917 cycle 5/6 to close the false-negative gap on `commands/wiki/lint.md` Layer 3b imperative recast; cycle 6 specifically strengthens TC-6.1 with line-start anchor to exclude inline backtick literal in rationale prose and adds TC-6.3 count assertion to detect partial revert); presence + imperative-keyword pin; byte equality is left to the existing `caller-html-literal-symmetry.test.sh` to preserve responsibility separation).
