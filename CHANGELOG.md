...
- Mitigated cross-orchestrator return-block implicit stop regression (#910)
  - Terminology note: this entry uses "site" with three distinct meanings disambiguated below — (a) **4 files** (the source files modified), (b) **4 grep targets** (test pin positions: 2 in `create.md` + 1 in `cleanup.md` + 1 in `ingest.md`), (c) **3 signaling layers** (caller HTML hint / sub-skill plain-text reminder / sub-skill HTML continuation comment).
  ...
  - Added cross-orchestrator regression test `hooks/tests/step0-immediate-bash-presence.test.sh` (assertions across **4 grep targets**; presence + imperative-keyword pin; byte equality is left to the existing `caller-html-literal-symmetry.test.sh` to preserve responsibility separation).
  - Updated `skills/rite-workflow/references/sub-skill-return-protocol.md` canonical contract to document the "prompt-side defense alone is insufficient" finding and the **3-layer** canonical signaling pattern (caller HTML hint / sub-skill plain-text reminder / sub-skill HTML continuation comment).
