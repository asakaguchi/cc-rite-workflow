...
> - PR #686 — `flow-state-update.sh` のマルチステート対応（per-session file API + `schema_version: 2`）
> - PR #747 — 旧形式 `.rite-flow-state` の自動 migration 機構（atomic rename + backup）
> - PR #748 / PR #756 (#749 follow-up) — lifecycle 4 hooks (`session-start` / `session-end` / `pre-compact` / `post-compact`) の自 session state 参照対応 + observability 改善
> - PR #750 — `pre-tool-bash-guard` / `post-tool-wm-sync` + `session-ownership.sh` helper のマルチステート整理
> - PR #751 — `scripts/` + `commands/` + `skills/` 配下の `.rite-flow-state` literal 直接参照を semantic API へ移行
> - PR #757 — 並行 2 セッション regression テスト (T-01) + Session Ownership 系 (#173/#206/#216/#558/#660) AC 再検証 (T-07) + AND 論理 8 種防御層
> - PR #759 — migration / atomic write / cleanup / crash resume テストスイート (T-03〜T-10)
>
...
[Wiki 経験則 #660](https://github.com/asakaguchi/cc-rite-workflow/issues/660) によれば、過去 9 件の Issue で導入した 8 種類の防御層 (declarative / sentinel / Pre-check / whitelist / Pre-flight / Step 0 / 4-site 対称化 / case arm) は AND 論理で組まれており、**`.rite-flow-state.active=true` という単一前提条件に依存** していた。当時の運用統計 (`.rite-stop-guard-diag.log` 直近 30 件中 28 件 = 93% が `EXIT:0 reason=not_active`) は撤去前の stop-guard.sh の挙動を示しており、**現行 6 hooks における同等の前提条件依存の有無は本設計選定時点では未測定**である。
...
| 関連 merged PR | #686 / #747 / #748 + #756 (#749 follow-up) / #750 / #751 / #757 / #759 |
...
| `--legacy-mode` flag | `migrate-flow-state.sh` の auto-migration 機構が `schema_version` キー欠落 or `< 2` の旧形式を起動時に自動検出 → atomic rename で新形式に変換するため、`--legacy-mode` flag は導入されなかった。代わりに `rite-config.yml` の `flow_state.schema_version: 1` で強制 fallback 可能（adapter pattern）だった（この fallback は #1458 で撤去済み — 下記「Rollback 戦略（撤去済み）」節参照） |
...
| 並行性の構造保証 | T-01 で 2 セッション同時起動を実測。lock 不要で race window が観測されないことを確認。Issue #660 で繰り返された race window 系列の root cause が解消された |
...
- 後続設計: [multi-session-worktree.md](./multi-session-worktree.md) — 本設計が分離した「状態」レイヤ（per-session flow-state）の上に「git 作業ツリー / カレントブランチ」レイヤの分離を完成させる Epic #1360（セッション別 Git worktree + EnterWorktree 自動入場）。本設計の `session_id` ベース構造を claim liveness 判定がそのまま再利用する
