...
if ! mkdir -p "$(dirname "$diag_log")" 2>/dev/null; then
  echo "WARNING: cannot create diag_log dir $(dirname "$diag_log") — diagnostic output for _resolve-flow-state-path.sh will be lost" >&2
fi
...
>
> **Forward note**: `wiki/ingest.md` / `cleanup.md` は依然 `--preserve-error-count` を維持しているが、これは旧 caller-side step 0/1 paradigm の名残であり同様に dead code 化済 (移行計画は ADR `docs/designs/parent-routing-unification.md` 参照)。両 site で挙動非対称となる経過期間が存在する点に注意 — 将来 reader が `error_count` を runtime 復活させた瞬間に create-interview だけ reset 経路を持つ非対称が出現するリスクは `hooks/tests/error-count-runtime-reference.test.sh` の dead code 機械検証で防御している。
