...
# verified-review M-1 (silent-failure-hunter) 対応で `_grep_count_safe` helper に統一し、stderr-tempfile +
...
# canonical 数が well-defined (Pre-flight if + cold-start elif + Return Output re-patch = 3)
# のため exact `== 3` で厳格化。site 追加が必要な refactor 時は閾値の同時更新が必須となる
# (silent additive misuse — 1 site 削除 + 1 site 追加で count=3 維持 — を catch する)。
if [ "$interview_patch_count" = "3" ]; then
  pass "TC-1: create-interview.md に 'flow-state-update.sh patch' が exactly 3 (実測=$interview_patch_count, Pre-flight if + Pre-flight cold-start elif + Return Output re-patch)"
else
...
# table column anchor を強化。
# 旧 regex `'\| `PREFLIGHT_CREATE_THEN_PATCH_FAILED=1` AND skip path'` は trailing column anchor `\|` を持たないため、
# 別 row の prose に「skip path」が追加された場合に誤検出する経路があった。
# 行末まで column を含む完全な row pattern (row 4 末尾の sentinel literal を含む) で pin する。
assert_grep "TC-2e-4: create-interview.md halt 判定表 row 4 (PREFLIGHT_CREATE_THEN_PATCH_FAILED AND skip path, full row pattern)" \
  "$INTERVIEW_MD" \
  '\| `PREFLIGHT_CREATE_THEN_PATCH_FAILED=1` AND skip path .*INTERVIEW_RETURN_PATCH_FAILED'
