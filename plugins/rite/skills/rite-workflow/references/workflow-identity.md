...
3. Stop の防御は (a) `commands/...` の prompt contract、(b) phase-transition-whitelist の許可遷移 graph、(c) `pre-tool-bash-guard.sh` の事前ガードに分散されている。`end_turn` を機械的に block する Stop hook は存在せず、protocol violation の検出は `start.md` ステップ 8.5 の retrospective grep に依拠する。
...
- 文章 enforcement に加え、flow-state + phase-transition-whitelist + HTML コメント sentinel の機械的 enforcement レイヤーに依拠する。
...
**Correct Pattern**:
- `commands/issue/create.md` ステップ 4.6 (Single Issue Terminal Completion) / ステップ 5.5 (Decompose Terminal Completion) の出力テンプレートを「flow-state deactivation → ✅ 完了メッセージ → 次のステップ → `<!-- [create:completed:{N}] -->` (HTML コメント最終行)」の順序で統一する。
