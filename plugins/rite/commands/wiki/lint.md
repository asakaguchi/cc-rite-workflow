...
if printf '%s' "$mode" | grep -qE '(^|[[:space:]])--auto([[:space:]]|$)'; then
  ...
  echo ""
  echo "> ⏭ MUST continue (turn を閉じない): caller (rite:wiki:ingest Phase 8.3-8.5 → Phase 9 Completion Report → 三点セット終端) を同 turn 内で完了してください。"
...
# Downstream reference: same file:Phase 8.3 — 本 emit 値 (lint_action=...) を {log_entry} 組み立て
...
Ingest 完了直後に呼ばれる場合、出力は最小化されます。**Issue #625 対応**: `--auto` モードの stdout は次の 3 要素を**この順序で必ず出力**します（三点セット、caller の implicit stop 防止）:
...
> ⏭ MUST continue (turn を閉じない): caller (rite:wiki:ingest Phase 8.3-8.5 → Phase 9 Completion Report → 三点セット終端) を同 turn 内で完了してください。
<!-- [lint:completed:auto] -->
```

**三点セット** (Issue #625、ingest.md Phase 9.1 #618 の canonical pattern を踏襲):

1. **6 フィールド 1 行** (`Lint: contradictions=N, ...`): ingest 側 Phase 8.3 step 2 の `^Lint: contradictions=` regex parser 互換 (形式不変)
2. **caller 継続 blockquote** (`> ⏭ MUST continue (turn を閉じない): ...`): LLM / user 両方に caller の Mandatory After 起動を促す user-visible reminder。LLM turn-boundary heuristic の fragile 誘発源 (plain-text `Lint:` 1 行で turn 終了) を抑止する。Issue #917 で imperative recast (現状報告 `継続中` → 命令形 + 否定形重ねがけ `MUST continue (turn を閉じない)`) に変更。本 blockquote は parent-routing pattern 移行 (ADR `docs/designs/parent-routing-unification.md`、PR #926) 後において Layer 3b plain-text reminder の **唯一の現役 site** (sub-skill-return-protocol.md "Canonical imperative strengthening coverage" 参照)。`commands/issue/create-interview.md` の同等 reminder は parent-routing pattern 移行で廃止済 (sub-skill 内製化)。
3. **HTML コメント sentinel** (`<!-- [lint:completed:auto] -->`): absolute last line — rendered view では不可視、`grep -F '[lint:completed:auto]'` で fixed string match 可能。ingest.md Item 0 routing dispatcher の **primary detection path** (Issue #618 AC-1) を発火させる

**検出件数が全て 0 の場合も含め、`--auto` モードの stdout は常にこの三点セットを出力する**（PR #564 レビュー MEDIUM #2 対応 — 旧仕様の「0 件なら stdout を空にして exit 0」は bash syntax error や未捕捉 fatal error での空 stdout を silent に "clean" と誤認する経路を生むため撤廃）。6 フィールド全てが 0 のときは `Lint: contradictions=0, stale=0, orphans=0, missing_concept=0, unregistered_raw=0, broken_refs=0` を 1 行目に出力する。これにより ingest 側の Phase 8.3 step 3 は「stdout が空 = Lint 実行失敗」として扱うことができる（stdout 空は unreachable な経路として定義される）。`unregistered_raw` は `n_warnings` に加算されないため、`n_unregistered_raw > 0` だけで `missing_concept` と他 4 カテゴリが 0 の場合も 6 フィールドを揃えたこの三点セットを出力する。

> **⚠️ MUST NOT (Issue #625, mirrors ingest.md Phase 9.1 MUST NOT #604/#561)**: 「ユーザー可視最終行 = `Lint: ...` plain text 1 行のみ」で `--auto` モード出力を終わらせてはならない。plain text 末尾は LLM の turn-boundary heuristic を誤発火させ、caller (rite:wiki:ingest) の Mandatory After Auto-Lint が未実行のまま turn が閉じる regression (Issue #625) を誘発する。**HTML コメント sentinel (`<!-- [lint:completed:auto] -->`) を absolute last line として必ず出力すること**。bare bracket 形式 (`[lint:completed:auto]` HTML コメント外) も禁止 — LLM turn-boundary heuristic を誤発火させる既知リスク (Issue #561 解消条件と同一)。
