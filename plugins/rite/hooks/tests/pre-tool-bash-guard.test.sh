...
# --------------------------------------------------------------------------
# TC-CRITICAL-1 (verified-review CRITICAL-1): Pattern 5 jq fail-fast 化
# silent-failure-hunter CRITICAL-1 — state file 破損 (jq parse error) 時に
# fail-OPEN (silent allow) から fail-CLOSED (BLOCK) に挙動を反転。
# 旧実装は STATE_PHASE="" / STATE_ACTIVE="false" で silent skip し、
# Mode B Bypass を許していた。
# --------------------------------------------------------------------------
echo "TC-CRITICAL-1: Pattern 5 jq fail-fast — corrupt state file + gh issue create → BLOCK"
tmpdir=$(mktemp -d)
mkdir -p "$tmpdir/.rite/sessions"
SID_C1="bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb"
echo "$SID_C1" > "$tmpdir/.rite-session-id"
# 破損した state file (JSON parse error を起こす binary noise)
printf '{this is not valid json\x00\xff' > "$tmpdir/.rite/sessions/${SID_C1}.flow-state"
# rite-config.yml で schema_version=2 を指定 (per-session path resolver 経由)
cat > "$tmpdir/rite-config.yml" << EOF
flow_state:
  schema_version: 2
EOF
rc=0
output=$(run_guard_with_cwd "$tmpdir" "gh issue create --title 'test'") || rc=$?
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
reason=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecisionReason // empty' 2>/dev/null)
if [ "$decision" = "deny" ] && [[ "$reason" == *"create-lifecycle-state-corrupt"* ]]; then
  pass "TC-CRITICAL-1: corrupt state file + gh issue create → BLOCK with state-corrupt reason (fail-closed)"
else
  fail "TC-CRITICAL-1: expected fail-closed deny with 'create-lifecycle-state-corrupt' reason, got decision=$decision reason=$reason"
fi
# WARNING が stderr に出ていることも確認
stderr_log=$(cat "$STDERR_FILE")
if [[ "$stderr_log" == *"jq .phase 抽出失敗"* ]] || [[ "$stderr_log" == *"jq .active 抽出失敗"* ]]; then
  pass "TC-CRITICAL-1: jq failure WARNING emitted to stderr (observability 維持)"
else
  fail "TC-CRITICAL-1: expected jq failure WARNING, got stderr: $(printf '%s' "$stderr_log" | head -3)"
fi
rm -rf "$tmpdir"

# Negative case: corrupt state file + non-gh-issue-create command → ALLOW
# (state corruption は gh issue create のみ block する、他コマンドは影響しない)
echo "TC-CRITICAL-1 negative: corrupt state file + non-gh-issue command → allow"
tmpdir=$(mktemp -d)
mkdir -p "$tmpdir/.rite/sessions"
echo "$SID_C1" > "$tmpdir/.rite-session-id"
printf '{broken json' > "$tmpdir/.rite/sessions/${SID_C1}.flow-state"
cat > "$tmpdir/rite-config.yml" << EOF
flow_state:
  schema_version: 2
EOF
rc=0
output=$(run_guard_with_cwd "$tmpdir" "ls -la") || rc=$?
decision=$(echo "$output" | jq -r '.hookSpecificOutput.permissionDecision // empty' 2>/dev/null)
if [ -z "$decision" ] || [ "$decision" != "deny" ]; then
  pass "TC-CRITICAL-1 negative: corrupt state + non-gh-issue → not blocked (scope limitation 維持)"
else
  fail "TC-CRITICAL-1 negative: expected allow for non-gh-issue command, got decision=$decision"
fi
rm -rf "$tmpdir"

# --------------------------------------------------------------------------
# TC-CRITICAL-2 (verified-review CRITICAL-2): ERR trap scope narrowing
# silent-failure-hunter CRITICAL-2 — ERR trap scope を Pattern 1-3 と Pattern 5 の
# 2 区間に限定 (中間区間と Pattern 4/charter-lint は trap scope 外)。
# Static pin: 3 つの DRIFT-CHECK ANCHOR が線形順序に存在することを確認。
# --------------------------------------------------------------------------
echo "TC-CRITICAL-2: ERR trap scope narrowing — DRIFT-CHECK ANCHORs static pin"
_disarm_line=$(grep -n 'DRIFT-CHECK ANCHOR: err_trap_disarm_after_pattern3' "$HOOK" | head -1 | cut -d: -f1)
_rearm_line=$(grep -n 'DRIFT-CHECK ANCHOR: err_trap_rearm_before_pattern5' "$HOOK" | head -1 | cut -d: -f1)
_final_disarm_line=$(grep -n 'DRIFT-CHECK ANCHOR: err_trap_disarm_after_pattern5' "$HOOK" | head -1 | cut -d: -f1)
if [ -n "$_disarm_line" ] && [ -n "$_rearm_line" ] && [ -n "$_final_disarm_line" ] && \
   [ "$_disarm_line" -lt "$_rearm_line" ] && [ "$_rearm_line" -lt "$_final_disarm_line" ]; then
  pass "TC-CRITICAL-2: 3 ERR-trap anchors exist in linear order (disarm_p3=$_disarm_line < rearm_p5=$_rearm_line < disarm_p5=$_final_disarm_line)"
else
  fail "TC-CRITICAL-2: ERR-trap anchors missing or out of order (disarm_p3=$_disarm_line, rearm_p5=$_rearm_line, disarm_p5=$_final_disarm_line)"
fi
unset _disarm_line _rearm_line _final_disarm_line

# --------------------------------------------------------------------------
# TC-C7 (verified-review C7): ERR-trap-disarm static pin
# pr-test-analyzer CRITICAL-5 — Pattern 4 / charter-lint strict BLOCK の fail-closed
# semantics を pin する。`trap - ERR` を経由しないと strict mode BLOCK が silent 0 に倒れる。
# --------------------------------------------------------------------------
echo "TC-C7: ERR-trap-disarm static pin (charter-lint strict BLOCK fail-closed 維持)"
# ERR trap install が WARNING + exit 0 emit する形式であること (fail-open + observability)
if grep -qE "trap.*ERR trap fired.*fail-open silent allow.*exit 0.*ERR" "$HOOK"; then
  pass "TC-C7: ERR trap install line matches WARNING + exit 0 fail-open pattern"
else
  fail "TC-C7: ERR trap install line drifted from expected fail-open + observability pattern"
fi
# disarm/rearm pairs が両方存在
if grep -cE '^trap - ERR$' "$HOOK" | grep -qE '^[2-9]$|^[0-9]{2,}$'; then
  pass "TC-C7: 'trap - ERR' disarm line appears 2+ times (after Pattern 3 + after Pattern 5)"
else
  _disarm_count=$(grep -cE '^trap - ERR$' "$HOOK")
  fail "TC-C7: 'trap - ERR' disarm line count = $_disarm_count (expected >= 2)"
fi
...
# --------------------------------------------------------------------------
# TC-HIGH-3 (verified-review SF HIGH-3): STAGED_FILES git failure WARNING
# --------------------------------------------------------------------------
echo "TC-HIGH-3 (verified-review HIGH-3): STAGED_FILES git diff failure WARNING static pin"
if grep -qE '_staged_err=\$\(mktemp' "$HOOK" \
   && grep -qE 'git diff --cached --name-only が失敗' "$HOOK"; then
  pass "TC-HIGH-3: STAGED_FILES stderr-tempfile + WARNING (silent IO suppression 防止)"
else
  fail "TC-HIGH-3: STAGED_FILES git failure handling が消失"
fi

# --------------------------------------------------------------------------
# TC-HIGH-4 (verified-review SF HIGH-4): CHARTER_FENCE_COUNT rc 区別
# --------------------------------------------------------------------------
echo "TC-HIGH-4 (verified-review HIGH-4): CHARTER_FENCE_COUNT grep rc=1 vs rc>=2 区別"
if grep -qE 'charter fence grep が IO エラー' "$HOOK" \
   && grep -qE '_fence_rc' "$HOOK"; then
  pass "TC-HIGH-4: CHARTER_FENCE_COUNT rc=1 (no match) と rc>=2 (IO error) を区別"
else
  fail "TC-HIGH-4: CHARTER_FENCE_COUNT rc 区別ロジックが消失"
fi

# --------------------------------------------------------------------------
# TC-HIGH-5 (verified-review SF HIGH-5): CHARTER_MSG_STRIPPED awk failure WARNING
# --------------------------------------------------------------------------
echo "TC-HIGH-5 (verified-review HIGH-5): CHARTER_MSG_STRIPPED awk failure WARNING static pin"
if grep -qE 'charter fence-strip awk が失敗' "$HOOK" \
   && grep -qE '_awk_err' "$HOOK"; then
  pass "TC-HIGH-5: CHARTER_MSG_STRIPPED awk failure WARNING + stderr-tempfile (silent suppression 防止)"
else
  fail "TC-HIGH-5: CHARTER_MSG_STRIPPED awk failure handling が消失"
fi
