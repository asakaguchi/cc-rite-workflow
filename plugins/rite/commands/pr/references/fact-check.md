...
> **セクション名について**: 名称は「外部仕様の検証結果」だが、本セクションは **両 Sub-Phase の検証結果** を統合して記録する (Internal Likelihood Claim の立証/降格も含む)。`種別` 列で sub-phase 出典を明示することで名称と内容の乖離を補う。
...
> **fix.md 互換性**: `### 外部仕様の検証結果` および `### 矛盾により除外された指摘` セクションは `### 全指摘事項` の**前**に配置する。fix.md Phase 1.2.1 は `### 全指摘事項` を起点にパースするため影響なし。VERIFIED findings の `推奨対応` 列へのソース URL 付記は column 4 のテキストとして無害にパースされる。

> **caller 側 (review.md / assessment-rules.md) との同期**: 本ファイルは Fact-Check Phase の仕様書 (source of truth) であり、review.md `#### Fact-Check Phase` の実行手順および assessment-rules.md の `Fact-Check exclusion` / `【外部仕様検証】` 出力フォーマットは本ファイルの内容に追従する必要がある。
