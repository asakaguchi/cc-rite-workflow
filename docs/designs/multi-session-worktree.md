...
| V-4 | `.rite-compact-state` が単一ファイル（セッション間 last-writer-wins）の既存ギャップ | 統一 root 化で顕在化しうる → per-session 化を S11（optional）に切り出し | ✅ 解消。S11（#1371 / PR #1381）で `.rite-compact-state` を per-session 化。`crash-resume.test.sh` 7 PASS が並行 SIGKILL 下の state 整合を担保 |
