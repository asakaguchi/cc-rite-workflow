...
非問題 5 sub-skill から抽出した正規構造の骨格を以下に示す。完全な canonical form (実装側の導入文・blockquote 含む **structurally equivalent な形式** — 同一テンプレート骨格を共有し sub-skill 固有の phase 名 / sentinel 値 / prose text のみ差し替える、byte-equivalent ではない) は `plugins/rite/commands/issue/parent-routing.md:325-357` および `plugins/rite/commands/issue/branch-setup.md:123-153` を SoT として参照すること。本 ADR は本 8 PR 構成での移行先テンプレートの骨格のみを示す:
...
**結果**: `verify-terminal-output.sh` は `--repo-root <path>` 引数で scope 限定可能 (`usage()` 関数内の `--repo-root` option ブロック参照、行番号は drift するため structural anchor で示す)。本 ADR では migration 期に `commands/` 配下に限定した検査に変更し、`docs/` / `CHANGELOG*` / `tests/` への誤検出を防ぐ。
...
- ~~IMP-2: TC-7a を `count >= 2` に強化~~ → 本 PR `parent-routing-pattern-interim.test.sh` TC-7a で `grep -cE '\[interview:error\].*halt' "$CREATE_MD"` >= 2 を実装済
- ~~IMP-3: TC-2 に `--if-exists` count >= 3~~ → 本 PR `parent-routing-pattern-interim.test.sh` TC-2h で `grep -cE '\-\-if-exists' "$INTERVIEW_MD"` >= 3 を実装済
- ~~IMP-4: `verify-terminal-output.sh` Check 3 を独立 grep に分割~~ → 本 PR `verify-terminal-output.sh` Check 3 で `completed` / `skipped` 独立 grep を実装済 (`error` は legacy OR-form として historical fixture 互換性のため維持)
