...
if ! bash {plugin_root}/hooks/flow-state-update.sh patch \
 ...
 --if-exists; then
 echo "WARNING: flow-state-update.sh patch (ingest_pre_lint) failed — sub-skill will run unphased, so retrospective workflow-incident correlation against ingest_pre_lint will not fire. Investigate the helper exit reason in stderr above before relying on this protection again." >&2
fi
