...
ORIG_BLH=$(git branch --list 2>/dev/null | sort | md5sum 2>/dev/null | awk '{print $1}')
echo "review_pre_state: branch=$ORIG_BR stash_count=$ORIG_SC branch_list_hash=$ORIG_BLH"
```

LLM は出力 3 値 (`ORIG_BR`, `ORIG_SC`, `ORIG_BLH`) を Phase 5.0.A の `post-review-state-verify.sh` 引数に literal substitute する。
...
| 重要度 | ファイル:行 | 内容 | 推奨対応 |
|--------|------------|------|----------|
| {CRITICAL/HIGH/MEDIUM/LOW-MEDIUM/LOW} | {file:line} | {WHAT: 何が問題か} + {WHY: なぜ問題か（影響・リスク・既存パターンとの比較）} | {FIX: 修正方法} + {EXAMPLE: コード例（該当時）} |
...
result_json=$(bash {plugin_root}/hooks/scripts/post-review-state-verify.sh \
  ...
  --auto-recover true 2>&1) || true
printf '%s\n' "$result_json"
```

スクリプトは stderr に WARNING を emit し、stdout に `{"drift":..., "type":..., "recovered":...}` を出力する。drift 検出時の処理は **non-blocking** (review flow は継続)、drift 結果は Phase 5.4 完了レポートに reflect される (orchestrator が `result_json` を会話 context に保持し、必要に応じて完了レポートで言及)。`workflow_incident_emit` 経路により、orchestrator 側 (`commands/issue/start.md` Phase 5.4.4.1) の grep detection で Issue 化 routing に乗る。
...
### 推奨事項（該当がある場合のみ）
<!-- Phase 5.1 で収集した recommendation_issue_candidates + Phase 5.3.0 で降格された Hypothetical findings がある場合のみ表示。0件の場合はこのセクション自体を省略 -->
