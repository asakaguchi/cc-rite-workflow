...
2. **caller 継続 blockquote** (`> ⏭ MUST continue (turn を閉じない): ...`): LLM / user 両方に caller の Mandatory After 起動を促す user-visible reminder。LLM turn-boundary heuristic の fragile 誘発源 (plain-text `Lint:` 1 行で turn 終了) を抑止する。Issue #917 で imperative recast (現状報告 `継続中` → 命令形 + 否定形重ねがけ `MUST continue (turn を閉じない)`) に変更し、Layer 3b plain-text reminder 強度を `commands/issue/create-interview.md` Return Output Format の `> ⏭ MUST continue (turn を閉じない):` と対称化。
