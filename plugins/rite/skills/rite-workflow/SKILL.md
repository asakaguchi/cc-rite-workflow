...
3 step 以上の sequential workflow (`pr:cleanup`, `pr:iterate`, `pr:open`, `pr:review`, `pr:fix`, `sprint:execute`, `wiki:ingest`, `wiki:lint` 等。列挙は例示で完全網羅ではない) を実行する際は、開始時に TaskCreate でステップ列を登録し、各 step 完了時に TaskUpdate で進捗を更新すること。TaskCreate を発行した最外側 skill の return 時点で TaskList を確認し、未完了タスクが残っている場合は未実行の最初の step に戻って実行を継続する (turn を終了しない)。全タスクが completed の場合のみ turn を終了する。nested sub-skill (例: `wiki:lint`) の return 時点では TaskUpdate のみ行い、turn 終了の判断は最外側 skill に委ねる。これにより skill ネスト時 (例: `pr:cleanup → wiki:ingest → wiki:lint`) の最内側 sentinel を turn 終了と誤認する事故を防ぐ。
