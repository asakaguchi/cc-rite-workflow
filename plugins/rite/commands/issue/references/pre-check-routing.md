...
|---|-------------|------------------------|
| 0 | **Routing dispatcher** (状態質問ではない): 直前の sub-skill return tag は何か? | grep the recent output (HTML comments included) for `[create:completed:{N}]` / (historical: `[interview:skipped]` / `[interview:completed]` / `[CONTEXT] INTERVIEW_DONE=1`)。**Historical (PR #1079)**: `[interview:*]` grep marker は旧 `create-interview.md` Return Output Format の FIRST 行として emit されていた plain-text marker。PR #1079 で `/rite:issue:create` は flat workflow に統合され、interview phase は `create.md` 内 inline で実行されるため、これらの grep marker は active な return form として emit されなくなった。歴史的に grep 経路は 4 種類だったが、現在は `[create:completed:{N}]` の検出のみが load-bearing。推奨形式は `grep -F '[create:completed:'` (fixed string で HTML コメント内 string もマッチ)。ERE を使う場合は `grep -E '\[create:completed:[0-9]+\]'` (`-E` flag 必須)。`[create:completed:{N}]` matched: run 🚨 Mandatory After Delegation self-check (Step 1/2 no-ops when marker is present, Step 3 is idempotent output)。tag が上記いずれでもない / 無い: 通常の Phase 進行中なので Item 1-3 を評価。未知 tag (unexpected return format): manual 停止して diag log を確認。**本 Item は YES/NO 集計から除外** — ルーティング前段として機能する。 |
