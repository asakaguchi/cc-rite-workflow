...
The contract is enforced across three active layers (Layer 2 was retired in #675; Layer 4 was added by Issue #923). Violating any active layer is a bug:
...
| 4. Mechanical enforcement (PostToolUse hook) | `plugins/rite/hooks/auto-fire-step0.sh` (matcher `Skill`) fires after Skill tool completion. Mapping `(pre-X phase) → (post-X phase, caller, next-action)` covers `ingest_pre_lint → ingest_post_lint` (caller `rite:wiki:ingest`) and `cleanup_pre_ingest → cleanup_post_ingest` (caller `rite:pr:cleanup`). The hook (1) patches flow-state to the post-X phase (idempotent with Layer 1 Step 0 / Step 1 via `--if-exists --preserve-error-count`) and (2) injects `hookSpecificOutput.additionalContext` into the next LLM turn input. Opt-out: `rite-config.yml` `workflow.auto_fire_step0.enabled: false` | `plugins/rite/hooks/auto-fire-step0.sh`, `plugins/rite/hooks/hooks.json` PostToolUse entry, `plugins/rite/hooks/tests/auto-fire-step0.test.sh`, `plugins/rite/hooks/tests/cross-orchestrator-step0-symmetry.test.sh` | active |
...
The LLM receives the continuation signal from three independent sources: the prompt itself (Layer 1), the **inline HTML comment + plain-text reminder + sub-skill HTML continuation comment** in the sub-skill output (Layer 3 = 3a + 3b + 3c の internal decomposition、下記 blockquote 参照)、and the PostToolUse hook's `hookSpecificOutput.additionalContext` injected into the next-turn input (Layer 4). The imperative strength of Layer 1 + Layer 3 全体 plus the mechanical fire of Layer 4 is therefore load-bearing — see the blockquote below.
...
> **⚠️ Layer 4 rationale — declarative limit と mechanical safety net** (Issue #923): Wiki experiential knowledge `Declarative enforcement で LLM の stop_reason: end_turn は抑制できない` で確定した通り、Layer 1 + Layer 3 (prompt-side declarative defense) は continuation 確率を上げるが完全抑制不可能。`/rite:pr:cleanup` chain で `wiki:lint` return / `wiki:ingest` return の 2 接続点での implicit stop は累積系列対策後も繰り返し観測されており、prompt-side imperative 強度の継続的引き上げでは構造的に到達不可能な領域がある。Layer 4 (`auto-fire-step0.sh` PostToolUse hook) はこの limit に対する mechanical safety net で、`(pre-X phase, Skill tool 完了) → 自動 post-X patch + additionalContext injection` の組合せにより end_turn 発生時の recovery 経路を機械化する。**Layer 4 は Layer 1/Layer 3 を置換しない** — prompt-side contract は依然として canonical で、Layer 4 は hook の matcher 未 fire (Skill ツール non-PostToolUse 環境) / opt-out (`workflow.auto_fire_step0.enabled: false`) / hook patch 失敗時の fallback として Layer 1 Step 0 / Step 1 が機能する三層構造を維持する。AC-1 (`/rite:pr:cleanup` 1 サイクル user 手動 `continue` 介入なしに自動完走) は Layer 1 + Layer 3 + Layer 4 の合算で probabilistic に達成され、完全達成されない場合は `workflow_incident` 経路で観測される縮退仕様。
...
>
> canonical 英語 keyword (3a / 3c) + Layer 1 prose は `hooks/tests/step0-immediate-bash-presence.test.sh` で **5 cross-orchestrator grep targets** (`commands/issue/create.md` Mandatory After Interview / Mandatory After Delegation の 2 site、`commands/pr/cleanup.md` Mandatory After Wiki Ingest の 1 site、`commands/wiki/ingest.md` continuation HTML comment の 1 site、`commands/wiki/ingest.md` Mandatory After Auto-Lint Step 0 prose の 1 site = 計 5 物理 site、Issue #917 で 4 → 5 に拡張) に加えて、補完的に **3 supplementary pin type categories** (`commands/issue/create-interview.md` 単一ファイル内: caller HTML literal positive 2 + anti-pattern revert 2 + plain-text reminder 1 = 計 5 supplementary assertion) を pin する。test scope は **物理 5 site (cross-orchestrator) + supplementary 3 pin type category (intra-file in `create-interview.md`, 5 assertion)** で、5 物理 site は主たる canonical site、補完 pin は asymmetric weakening 検出を担当する主従構造を持つ。粒度の数え方は 3 種類混在するため混同しないこと: **5** = 物理 site 数 (cross-orchestrator、Issue #917 で 4 → 5)、**3** = pin type category 数 (intra-file)、**5** = supplementary assertion 数 (intra-file)。**Granularity-mixing prohibition**: `合計 8 site` のような単純加算 framing は粒度差を曖昧化するため使用しない (この bold ラベル "**Granularity-mixing prohibition**" は CHANGELOG.md / CHANGELOG.ja.md / `hooks/tests/step0-immediate-bash-presence.test.sh` のコメント等から bold 形式の grep target として参照される anchor — Issue #917 cycle 5 で SoT に追加、cycle 6 で参照側 4 site を bold 形式に統一)。

## Relationship to Workflow Incident Detection

When the contract is violated in practice — the user types `continue` to recover — the orchestrator MAY emit the `auto_continuation_failed` sentinel via `plugins/rite/hooks/workflow-incident-emit.sh` so the incident is auto-registered as an Issue via Phase 5.4.4.1. This is an **optional observability sentinel** (MAY) — it does not enforce the contract but records violations for later diagnosis. The detection heuristic has false-positive risk and is out of scope for Issue #525 MUST requirements. The active layers (Layer 1 + Layer 3 + Layer 4) above are the actual enforcement; the sentinel is observability. See `docs/SPEC.md` "Sub-skill Return Auto-Continuation Contract" section for the full specification.
