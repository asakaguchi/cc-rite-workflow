...
| Input | 解決 |
|-------|------|
| 明示 `#N` | N を Issue または PR 番号として扱う |
| (なし) | 現在ブランチ名を `git rev-parse --abbrev-ref HEAD` で取得し、`rite-config.yml` の `branch.pattern`（既定 `{type}/issue-{number}-{slug}`）から Issue 番号を導出 → `gh pr list --head <branch> --json number,title,body` で PR を特定 |
...
2. **Issue 本文**: `gh issue view {number} --json title,body` → 問題・受入基準・「なぜ存在したか」。
3. **PR 本文**: `gh pr view {number} --json title,body` → 解決策の経緯・設計理由。
