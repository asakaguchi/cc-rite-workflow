...
- Mitigated cross-orchestrator return-block implicit stop regression (#910)
  ...
  - Added cross-orchestrator regression test `hooks/tests/step0-immediate-bash-presence.test.sh` (19 assertions: TC-1+2+3 for **11 main pins** — `create.md` ×4 + `cleanup.md` ×3 + `ingest.md` ×4 — plus TC-4 for **3 cross-file count assertions** and TC-5 for **5 supplementary assertions** over `create-interview.md` caller HTML literal — 2 caller HTML literal pins + 2 anti-pattern revert checks + 1 plain-text reminder pin — for complementary asymmetric-weakening detection; presence + imperative-keyword pin; byte equality is left to the existing `caller-html-literal-symmetry.test.sh` to preserve responsibility separation).
