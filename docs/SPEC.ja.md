...
| `wiki-query-inject.sh` | `/rite:pr:open` Step 1 (work memory init、旧 start.md ステップ 2.6 pre-#1136) / `implement.md` Phase 5.0.W / `pr/review.md` Phase 4.0.W / `pr/fix.md` Phase 0.5.W | 現在の Issue title/body に対して `/rite:wiki:query` を走らせ、マッチする heuristics を注入する。local の wiki ブランチが不在（fresh clone / 別 worktree）なら `origin/{wiki_branch}` から読む |
...
| フック | トリガ | アクション |
|--------|-------|-----------|
| `wiki-query-inject.sh` | `/rite:pr:open` Step 1（work memory 初期化、旧 start.md ステップ 2.6 pre-#1136）、`implement.md` Phase 5.0.W（実装）、`pr/review.md` Phase 4.0.W（レビュー）、`pr/fix.md` Phase 0.5.W（修正） | 現在の Issue タイトル/本文に対して `/rite:wiki:query` を実行し、マッチした経験則を注入 |
