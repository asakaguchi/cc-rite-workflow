...
| `{command_name}` | Command path | `/rite:open`, `/rite:pr-create` |
| `{phase}` | Phase ID (see [phase-mapping.md](./phase-mapping.md)) | `implement` |
| `{phase_detail}` | Detail state | `実装作業中`, `PR 作成完了` |
| `{next_command}` | Next command | `/rite:pr-create`, `/rite:review #42` |
...
| `/rite:pr-create` | `/rite:review #123` | `PR 作成完了、レビュー準備完了` |
| `/rite:review` (ok) | `/rite:ready` | `指摘なし、Ready for review に変更可能` |
| `/rite:review` (issues) | `/rite:fix` | `要修正の指摘あり、修正が必要` |

**PR number**: Replaced with actual number after `/rite:pr-create`. If retrieval fails, record `/rite:review` without number (fallback: auto-search).

**Design**: Each command reads from/writes to work memory. `/clear` can be executed anytime; `/rite:recover` recovers state.
...
Tracks the review-fix loop count. Updated by `/rite:review` Phase 6.2 after each review cycle.
...
**Update timing**: `/rite:review` Phase 6.2 reads the current value, increments by 1 (or sets to `1` if absent), and writes back alongside the review history.
...
Added after `/rite:review` as a new comment or section.
