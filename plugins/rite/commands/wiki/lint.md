...
```bash
# --auto モードでは Phase 9.2 の 2 行出力 (6 フィールド 1 行 + HTML コメント sentinel) を必ず出力する。
...
if printf '%s' "$mode" | grep -qE '(^|[[:space:]])--auto([[:space:]]|$)'; then
 # Phase 9.2 contract: Lint 1 行 + HTML sentinel の 2 行を出力
...
| エラー | 対処 | Phase |
|--------|------|-------|
| `wiki.enabled: false` | 早期 return (`--auto` モード時は Phase 9.2 の 2 行出力 (6 フィールド 0 件 1 行 + HTML コメント sentinel) を出力後 exit 0、それ以外は警告のみ exit 0) | Phase 1.1 |
| GNU date 非互換環境 | Phase 4 skip（exit 0 + WARNING） | Phase 1.2 |
| Wiki 未初期化 | `/rite:wiki:init` を案内 (`--auto` モード時は Phase 9.2 の 2 行出力 (6 フィールド 0 件 1 行 + HTML コメント sentinel) を出力後 exit 0) | Phase 1.3 |
