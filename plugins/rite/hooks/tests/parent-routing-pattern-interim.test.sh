...
#         + 4 sentinel literal の dispatcher grep target pin (PR #926 verified-review Important #4)
#
...
#
# Known limitation (PR #926 verified-review M10 / M13 対応): hardcoded `>= 3` は将来の正当な refactor
...
#
# Known limitation (PR #926 verified-review M9 対応): `grep -A 1` 隣接 pin は **cold-start branch 内**
...
echo
echo "=== TC-7: caller-side [interview:error] halt rule presence (PR #926 verified-review Important #4 対応) ==="
...
if [ ! -f "$PRE_CHECK_ROUTING_MD" ]; then
  ...
  # create.md には Halt rule (line 66 付近) と Phase 1 return branch (line 222 付近) の 2 箇所で
  # `[interview:error]` halt prose が存在する。
  if grep -qE '\[interview:error\].*halt' "$CREATE_MD"; then
    pass "TC-7a: create.md に '[interview:error] ... halt' prose が存在 (caller-side halt rule の silent weakening 検出)"
  else
    fail "TC-7a: create.md に '[interview:error] ... halt' prose が見つからない (caller-side halt rule が prose ごと削除された可能性)"
  fi
