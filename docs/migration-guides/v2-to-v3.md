...
| `commands/issue/references/workflow-incident-detection.md` | `start.md` ステップ 8.5 SoT |
...
| `start.md` ステップ 8.5 (Workflow Incident Detection) | sentinel grep → AskUserQuestion → Todo Issue 自動起票 |
| `rite-config.yml` の `workflow_incident:` key | opt-out 設定 |

**何が変わったか**: 旧機構は失敗 (Skill load 失敗 / hook 異常終了 / Projects 同期失敗 / Wiki ingest skip / gitignore drift 等) を `[CONTEXT] WORKFLOW_INCIDENT=1; ...` sentinel として emit し、ステップ 8.5 が conversation context を grep して tracking Issue を自動起票していました。v3 では各 caller が **plain な `WARNING` / `ERROR` を stderr に出力** するだけになり、LLM が会話コンテキストで surface します。中断した作業は `/rite:resume` で復帰します (多層防御 → 単層 + ユーザー操作)。
