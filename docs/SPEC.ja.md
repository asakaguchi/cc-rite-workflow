...
|------|------|--------------|------|
| Fix-side post-loop | `fix.md` Phase 4.3 (「Automatic Separate Issue Creation」) | **#1136 で削除済** | Phase 4.3 全体・`[fix:issues-created:N]` sentinel・`review.separate_issue_creation.*` config keys を撤去。`/rite:pr:fix` のレビュー・フィックスループ内では reviewer recommendation は per-finding で Phase 2.1 menu (fix / accept / reply) として処理され、post-loop auto-creation は無い。 |
...
>
> 残存する言語関連制御は documentation 側の規約のみ — kept-English term リスト (Issue / PR / Sprint / Iteration / finding / fingerprint / severity 等) と「ドキュメント vs 直書き UI 文言」の使い分けは `docs/i18n-style-guide.md` を参照。`rite-config.yml` の `language` 設定 (live) は commit message の言語のみを制御し、runtime UI message catalog を選択しない (#1117 以後そのような catalog 自体が存在しない)。
