...
Layers 1, 3, 4, 5 are complementary. Layers 2 and 6 are retired (#675) and shown above only as historical context — the active enforcement is the prompt contract (Layer 1) plus the caller HTML hint (Layer 3); pre-check list (Layer 4) and completion message (Layer 5) remain as orchestrator-side reinforcements. Weakening any active layer (e.g., relaxing Layer 1 imperative phrasing without compensating at Layer 3) re-opens the original #525 failure mode. Issue #910 added the Layer 3 sub-layer decomposition (3a/3b/3c) and tightened the imperative phrasing across Layer 1 + Layer 3 sites after dogfooding revealed the LLM turn-boundary heuristic still fires post-#675.
