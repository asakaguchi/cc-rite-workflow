...
halt 判定表の row 1〜4 のいずれかが該当する場合 (PREFLIGHT_CREATE_FAILED 単独経路、`PREFLIGHT_PATCH_FAILED` + `INTERVIEW_RETURN_PATCH_FAILED` の AND、`PREFLIGHT_CREATE_THEN_PATCH_FAILED` + `INTERVIEW_RETURN_PATCH_FAILED` の AND、`PREFLIGHT_CREATE_THEN_PATCH_FAILED` + skip path) は silent corrupt audit-trail よりも明示 error を優先する。transient FS pressure 等の稀なケースが対象で、通常運用では発火しない。
