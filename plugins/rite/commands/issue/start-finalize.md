...
```bash
# Sub-shell でラップして trap を block scope に閉じる (F-09: trap leak 防止)。
# block 全体で `set -o pipefail` を有効化することで、`gh api graphql | jq` pipeline の
# gh 失敗時に jq の exit が dominant にならず gh の rc を捕捉できるようにする (F-02)。
# path-declare → trap → mktemp の順序で race window を排除する (F-08)。
# stderr の改行を空白化して sentinel grep の後続行 pick up を防ぐ (F-14)。
(
  if [ "{pr_number}" != "0" ] && [ -n "{pr_number}" ]; then
    ...
    if [ "$PROJECTS_ENABLED" = "true" ] && [ -n "$PROJECT_NUMBER" ]; then
      ...
      if [ "$PR_IS_DRAFT" = "false" ]; then
        # pipefail 下で gh api graphql | jq pipeline の前段失敗を捕捉する (F-02)
    ...
    issue(number: $number) {
      projectItems(first: 10) {
        nodes {
          ...
          bash {plugin_root}/hooks/workflow-incident-emit.sh \
            --type projects_status_in_review_missing \
            --details "Issue #{issue_number} Workflow Termination Step 0: gh api graphql failed (rc=$gh_gql_rc, stderr=$gql_err_oneline)" \
            --root-cause-hint "gh_api_failure_at_final_fallback" \
            --pr-number {pr_number} >&2 || true
          CURRENT_STATUS=""
        fi
