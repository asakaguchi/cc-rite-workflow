...
加えて以下も PR-7 で対応する:
- IMP-2: `parent-routing-pattern-interim.test.sh` TC-7a を `count >= 2` に強化し create.md "Halt rule" (Sub-skill Return Protocol section) + Phase 1 return branch (`[interview:error]` return-branch bullet) を独立 pin (semantic anchor、行番号 drift surface を回避)
