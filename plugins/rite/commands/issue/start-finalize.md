...
- `{owner}`, `{repo}` *(intra-phase only — Prerequisites として上流伝播不要)* — **Workflow Termination Step 0 のみ** sub-shell 内で `gh repo view --json owner --jq '.owner.login'` / `--json name --jq '.name'` を直接実行して `$_owner` / `$_repo` に local 束縛する設計 (silent skip 経路の遮断目的)。**Phase 5.5.1 は `projects-status-update.sh` への delegation 経路** で、`{owner}`/`{repo}` placeholder は orchestrator が substitute する (callsites.md SoT に従う)。本項目は Placeholder Legend の宣言性維持と将来他 phase での再利用に備えて残す。
...
>
> **`$status_result` 変数の setup**: 下記 emit case 句は `$status_result` を参照するが、その上流 `status_json=$(...)` / `status_result=$(printf '%s' "$status_json" | jq -r '.result // "failed"')` は Callsite 2 の bash literal SoT (上記 Module link 参照) で定義される。同変数 setup は `pr/ready.md` Phase 4.2 minimal skeleton にも同型で出現するため、本 emit case 句はその setup ブロックの直後に append される前提で読むこと。
>
> ```bash
  ...
  if [ "{pr_number}" != "0" ] && [ -n "{pr_number}" ]; then
    ...
    if [ "$PROJECTS_ENABLED" = "true" ] && [ -n "$PROJECT_NUMBER" ]; then
      ...
      _finalize_step0_cleanup() {
        rm -f "${pr_view_err:-}" "${gql_err:-}" "${jq_err:-}"
      }
      ...
      if ! _owner=$(gh repo view --json owner --jq '.owner.login' 2>/dev/null) || [ -z "$_owner" ]; then
        echo "[rite] ⚠️ Workflow Termination Step 0: gh repo view --json owner に失敗 — AC-8 検知を skip" >&2
        bash {plugin_root}/hooks/workflow-incident-emit.sh \
          --type projects_status_in_review_missing \
          --details "Issue #{issue_number} Workflow Termination Step 0: gh repo view --json owner failed (AC-8 detection skipped)" \
          --root-cause-hint "gh_repo_view_owner_failed" \
          --pr-number {pr_number} >&2 || true
        _owner=""
      fi
      if ! _repo=$(gh repo view --json name --jq '.name' 2>/dev/null) || [ -z "$_repo" ]; then
        echo "[rite] ⚠️ Workflow Termination Step 0: gh repo view --json name に失敗 — AC-8 検知を skip" >&2
        bash {plugin_root}/hooks/workflow-incident-emit.sh \
          --type projects_status_in_review_missing \
          --details "Issue #{issue_number} Workflow Termination Step 0: gh repo view --json name failed (AC-8 detection skipped)" \
          --root-cause-hint "gh_repo_view_name_failed" \
          --pr-number {pr_number} >&2 || true
        _repo=""
      fi
      # _owner / _repo が空の場合、後段の gh api graphql は空文字列 substitute され失敗するため、
      # incident emit 経路に流れる (AC-8 検知が silent skip にならないことが保証される)。
...
**Step 0 non-blocking 保証**: 検知ロジックは best-effort で、jq parse 失敗は silent fall-through するが、**gh API 自体の失敗は `gh_api_failure_at_final_fallback` root_cause_hint で incident sentinel を emit する**。Workflow Termination 自体は Step 1 に必ず到達する。silent fall-through で最終 fallback 層が消失する経路は Wiki 経験則 `silent_omit_disables_defense_chain` に該当する。
