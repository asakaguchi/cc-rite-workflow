...
- **`sources` 配列追記**: 新しい Raw Source への参照を必ず追加する。追加する各エントリは `- type: "{type}"` / `  ref: "raw/{type}/{filename}"` の形式とし、**`ref` は必ず Raw Source の wiki-root 起点ファイルパス（フルパス）**にする。raw frontmatter の `source_ref` フィールド値（PR 識別子、例: `pr-1143`）を `ref` に転記してはならない（ステップ 5.3 `{source_ref}` 行の dual-use 警告と同一契約。Issue #1178）
...
3. **新規 Wiki ページの作成** (ステップ 4 で新規決定): `{plugin_root}/templates/wiki/page-template.md` を Read で読み、ステップ 5.3 のプレースホルダーを置換した内容を Write:
   ...
   > **複数 Raw Source からの作成**: page-template.md の `sources:` は単一スロット（`{source_type}`/`{source_ref}` 各 1 個）のみ。multi-cycle PR 等で複数の Raw Source を 1 ページに統合する場合は、Write 後に Edit で `- type: "{type}"` / `  ref: "raw/{type}/{filename}"` エントリを追加する。**追加・置換するすべての `ref` は Raw Source のフルパス**であり、raw frontmatter の `source_ref` フィールド値（PR 識別子）ではない（ステップ 5.3 `{source_ref}` 行の dual-use 警告と同一契約。Issue #1178）。
4. **既存 Wiki ページの更新** (ステップ 4 で更新決定): 対象ページを Read で読み、Edit で `## 詳細` 追記・`updated` 更新・`sources` 配列追記。`n_pages_updated` を +1 する。**`sources` に追記する各 `ref` は必ず Raw Source のフルパス `raw/{type}/{filename}`**（PR 識別子 `pr-NNNN` 禁止。ステップ 4.2 / 5.3 と同一契約）
...
| `{source_ref}` | Raw Source の wiki-root 起点ファイル相対パス（例: `raw/reviews/20260413T...md`）。template 側で `../../` prefix を hardcode するため、placeholder 値自体には prefix を含めない。**⚠️ raw frontmatter の `source_ref` フィールド値（PR 識別子、例: `pr-1143`）をそのまま使ってはならない** — page の `sources[].ref` は常に Raw Source の**ファイルパス** `raw/{type}/{filename}` であり、PR 識別子ではない（同名 placeholder と raw フィールドの dual-use 混同による drift。詳細は anti-pattern `placeholder-dual-use-resolution-drift`）。lint はこの `ref` をフルパスで raw と突合するため、PR 識別子だと raw→page 追跡が切れ false `missing_concept` を量産する（Issue #1178 実測） |
