...
`MULTI_SESSION_ENABLED=false`（デフォルト）のときはステップ 2 で従来の Step 2.2 / 2.3 をそのまま実行し、挙動は現行と完全一致する。
...
> **ステップ 2.2 / 2.3 の分岐**: ステップ 1.4 の `[CONTEXT] MULTI_SESSION_ENABLED=` marker を読む。`false`（デフォルト）→ 下記 **2.2 / 2.3**（従来動作、現行と完全一致）を実行。`true` → **2.2-W / 2.3-W**（セッション worktree、下記）を実行し、従来の 2.2 / 2.3 は **置換**してスキップする。
...
その後 `EnterWorktree` ツールを `path: {wt_path}` で呼び出す（`{wt_path}` は 2.2-W の `WT_CASE` marker の `path=` 値。EnterWorktree のツール側ガード「ユーザー / プロジェクト指示で明示された場合のみ」は `multi_session.enabled: true`（ユーザーの明示的 opt-in）+ 本コマンド定義の明示指示で満たす）。
