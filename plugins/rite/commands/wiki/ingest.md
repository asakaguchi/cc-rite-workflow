...
> **Layer 4 — Mechanical enforcement (`auto-fire-step0.sh`)**: PostToolUse hook `plugins/rite/hooks/auto-fire-step0.sh` (matcher `Skill`) fires after `rite:wiki:lint --auto` returns. When the pre-invocation `.phase = ingest_pre_lint`, the hook automatically patches flow state to `ingest_post_lint` (idempotent with Step 0 / Step 1 via `--if-exists --preserve-error-count`) and injects `additionalContext` into the next LLM turn input via `hookSpecificOutput`. This mechanical safety net addresses the limit documented in Wiki experiential knowledge — declarative enforcement (HTML sentinel / caller hint / Step 0 bash literal) raises continuation probability but cannot fully suppress LLM `stop_reason: end_turn`. Step 0 through Step 3 below remain canonical: they are the documented prompt-side contract, idempotent with the hook's patch, and serve as fallback when the hook is opt-out via `rite-config.yml` `workflow.auto_fire_step0.enabled: false` or when Claude Code's matcher does not fire for Skill invocations in a given environment. See `plugins/rite/skills/rite-workflow/references/sub-skill-return-protocol.md` for the full layer model.
