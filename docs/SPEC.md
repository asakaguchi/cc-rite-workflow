...
|----------|-------|-----------------|-------|
| Required (11) | `active` | `flow-state.sh set` | `true` while a workflow is in flight |
| Required | `issue_number` | `flow-state.sh set` | The Issue under work |
| Required | `branch` | `flow-state.sh set` | Feature branch name |
| Required | `phase` | `flow-state.sh set` | Current orchestrator phase (flat enum: `init` / `branch` / `plan` / `implement` / `lint` / `pr` / `review` / `fix` / `ready` / `ready_error` / `completed`) |
| Required | `previous_phase` | `flow-state.sh set` | Auto-populated from outgoing `phase` value |
...
| Required | `updated_at` | `flow-state.sh set` (every write) | ISO 8601 with UTC offset (`+00:00`); generated by `date -u +"%Y-%m-%dT%H:%M:%S+00:00"` (cross-platform deterministic). Note: human-facing logs elsewhere may be JST; the persisted state field is UTC |
...
| Optional | `schema_version` | `flow-state.sh set` | `2` for the per-session structure; absent or `< 2` triggers migration |
