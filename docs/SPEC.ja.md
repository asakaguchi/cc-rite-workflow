...
| `review.*` | `loop.*`（convergence_monitoring / auto_propagation_scan / pre_commit_drift_check）、`doc_heavy.*`、`fact_check.*`（`use_context7` 含む）、`debate.*`、`security_reviewer.*`、`confidence_threshold`、`observed_likelihood_gate.*` / `fail_fast_first.*`（すべて #506）。`separate_issue_creation.*` キーは #1136 で完全削除され、`[fix:issues-created:N]` sentinel と `fix.md` Phase 4.3 も併せて撤去された (CONFIGURATION.ja.md の Deprecated 注記を参照) |
...
旧 Phase 7 — reviewer の「別 Issue として作成」推奨から自動 Issue を作成する経路 (`source: pr_review`) — は `fix.md` Phase 4.3 (「Automatic Separate Issue Creation」) 内で `plugins/rite/scripts/create-issue-with-projects.sh` を直接呼び出していました。#1136 で本機構全体 (Phase 4.3 / `[fix:issues-created:N]` sentinel / `review.separate_issue_creation.*` config keys) を撤去しました。reviewer 推奨はループ内のみで処理 — Phase 2.1 の選択肢 `fix code` / `accept (acknowledge only)` / `reply only` から選び、別 Issue として切り出したい懸念事項は `/rite:issue:create` で手動起票します (必要に応じて `/rite:investigate` で事前に fact-check)。`scripts/create-issue-with-projects.sh` ヘルパー自体は引き続き Issue 作成の canonical な経路として手動利用できます。
